...
export const fieldPreplans = sqliteTable("field_preplans", {
  id: text("id").primaryKey(),
  ...
  completenessStatus: text("completeness_status").notNull().default("legacy"),
  revisionNumber: integer("revision_number").notNull().default(1),
  lastVerifiedAt: text("last_verified_at"),
  nextReviewDate: text("next_review_date"),
  constructionProfile: text("construction_profile").notNull().default("{}"),
  occupancyProfile: text("occupancy_profile").notNull().default("{}"),
  fireFlowProfile: text("fire_flow_profile").notNull().default("{}"),
  targetHazardLevel: text("target_hazard_level").notNull().default("low"),
  targetHazardOverride: integer("target_hazard_override").notNull().default(0),
  targetHazardReasons: text("target_hazard_reasons").notNull().default("[]"),
  createdBy: text("created_by").notNull(),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  updatedBy: text("updated_by").notNull(),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [index("field_preplan_location_idx").on(table.latitude, table.longitude)]);

export const fieldPreplanLevels = sqliteTable("field_preplan_levels", {
  id: text("id").primaryKey(),
  preplanId: text("preplan_id").notNull().references(() => fieldPreplans.id),
  name: text("name").notNull(),
  shortLabel: text("short_label").notNull(),
  layerType: text("layer_type").notNull(),
  floorIndex: integer("floor_index"),
  gradeDesignation: text("grade_designation").notNull().default("at_grade"),
  sortOrder: integer("sort_order").notNull().default(0),
  isDefault: integer("is_default", { mode: "boolean" }).notNull().default(false),
  respondVisible: integer("respond_visible", { mode: "boolean" }).notNull().default(true),
  hidden: integer("hidden", { mode: "boolean" }).notNull().default(false),
  archived: integer("archived", { mode: "boolean" }).notNull().default(false),
  backgroundType: text("background_type").notNull().default("none"),
  backgroundAssetId: text("background_asset_id"),
  backgroundTransform: text("background_transform").notNull().default("{}"),
  ...
  uniqueIndex("field_preplan_level_label_idx").on(table.preplanId, table.shortLabel),
  index("field_preplan_level_order_idx").on(table.preplanId, table.archived, table.sortOrder),
]);

export const fieldPreplanSpaces = sqliteTable("field_preplan_spaces", {
  id: text("id").primaryKey(),
  preplanId: text("preplan_id").notNull().references(() => fieldPreplans.id),
  levelId: text("level_id").notNull().references(() => fieldPreplanLevels.id),
  displayName: text("display_name").notNull(),
  roomNumber: text("room_number").notNull().default(""),
  aliases: text("aliases").notNull().default("[]"),
  cadKeywords: text("cad_keywords").notNull().default("[]"),
  spaceType: text("space_type").notNull().default("room"),
  geometry: text("geometry").notNull(),
  coordinateSpace: text("coordinate_space").notNull().default("floor_plan"),
  accessNotes: text("access_notes").notNull().default(""),
  fireProtectionNotes: text("fire_protection_notes").notNull().default(""),
  ...
  updatedBy: text("updated_by").notNull(),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [index("field_preplan_space_level_idx").on(table.preplanId, table.levelId, table.archived)]);

export const fieldPreplanAlerts = sqliteTable("field_preplan_alerts", {
  id: text("id").primaryKey(),
  preplanId: text("preplan_id").notNull().references(() => fieldPreplans.id),
  levelId: text("level_id").references(() => fieldPreplanLevels.id),
  spaceId: text("space_id").references(() => fieldPreplanSpaces.id),
  alertType: text("alert_type").notNull(),
  title: text("title").notNull(),
  instructions: text("instructions").notNull().default(""),
  severity: text("severity").notNull().default("informational"),
  displayOrder: integer("display_order").notNull().default(0),
  pinToRespond: integer("pin_to_respond", { mode:"boolean" }).notNull().default(false),
  effectiveAt: text("effective_at"),
  expiresAt: text("expires_at"),
  expirationAction: text("expiration_action").notNull().default("require_verification"),
  verificationRequired: integer("verification_required", { mode:"boolean" }).notNull().default(false),
  ...
  updatedBy: text("updated_by").notNull(),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [index("field_preplan_alert_lifecycle_idx").on(table.preplanId, table.archived, table.expiresAt)]);

export const fieldPreplanHazmat = sqliteTable("field_preplan_hazmat", {
  id: text("id").primaryKey(),
  preplanId: text("preplan_id").notNull().references(() => fieldPreplans.id),
  levelId: text("level_id").references(() => fieldPreplanLevels.id),
  spaceId: text("space_id").references(() => fieldPreplanSpaces.id),
  ...
  unNaNumber: text("un_na_number").notNull().default(""),
  ergGuideNumber: text("erg_guide_number").notNull().default(""),
  ...
  updatedBy: text("updated_by").notNull(),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [index("field_preplan_hazmat_lookup_idx").on(table.preplanId, table.unNaNumber, table.archived)]);

export const fieldPreplanAssets = sqliteTable("field_preplan_assets", {
  id: text("id").primaryKey(),
  preplanId: text("preplan_id").notNull().references(() => fieldPreplans.id),
  levelId: text("level_id").references(() => fieldPreplanLevels.id),
  ...
  updatedBy: text("updated_by").notNull(),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [index("field_preplan_asset_preplan_idx").on(table.preplanId, table.levelId, table.category)]);

export const fieldPreplanRevisions = sqliteTable("field_preplan_revisions", {
  id: text("id").primaryKey(),
  preplanId: text("preplan_id").notNull().references(() => fieldPreplans.id),
...
}, (table) => [uniqueIndex("field_preplan_revision_number_idx").on(table.preplanId, table.revisionNumber)]);
