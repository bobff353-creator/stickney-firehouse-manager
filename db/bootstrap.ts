...
const boxCardSeedVersion = "stickney-box-cards-editable-2026-07-19";

const stickneyBoxCards = [
  ["sfd-box-300-e", "Structure Fire — East of Ridgeland Avenue", "300-E", "East of Ridgeland Avenue", 1, "2026-02-01", "2026-01-10"],
  ["sfd-box-300-w", "Structure Fire — West of Ridgeland Avenue", "300-W", "West of Ridgeland Avenue", 2, "2026-02-01", "2026-04-10"],
  ["sfd-box-399", "Ambulance (MCI / ASHER)", "399", "Entire Village", 3, "2025-03-01", "2025-01-23"],
  ["sfd-box-1000", "Hazardous Materials", "1000", "Entire Town", 4, "2025-03-01", "2025-11-17"],
  ["sfd-box-303-e", "Extrication / Pin-In — East of Ridgeland", "303-E", "East of Ridgeland Avenue", 5, "2025-03-01", "2025-01-23"],
  ["sfd-box-303-w", "Extrication / Pin-In — West of Ridgeland", "303-W", "West of Ridgeland Avenue", 6, "2025-03-01", "2025-01-23"],
  ["sfd-box-305", "Technical Rescue", "305", "Entire Town", 7, "2025-03-01", "2025-01-17"],
  ["sfd-box-301", "Fire Investigators", "301", "Entire Town", 8, "2025-03-01", "2025-01-17"],
  ["sfd-box-306", "Divers", "306", "Entire Town", 9, "2025-03-01", "2025-01-17"],
] as const;
...
async function seedBoxCards(db: Awaited<ReturnType<typeof getDatabaseBinding>>) {
  ...
  await db.batch(stickneyBoxCards.map((card) => db.prepare(
    "INSERT INTO box_cards (id, title, address, box_number, access_notes, details, department, document_url, document_page, effective_date, review_date, status, created_by, created_at, updated_by, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'Stickney', '/stickney-box-cards.pdf', ?, ?, ?, 'Active', 'Box card PDF import', CURRENT_TIMESTAMP, 'Box card PDF import', CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET title = excluded.title, address = excluded.address, box_number = excluded.box_number, department = excluded.department, document_url = excluded.document_url, document_page = excluded.document_page, effective_date = excluded.effective_date, review_date = excluded.review_date, updated_by = excluded.updated_by, updated_at = CURRENT_TIMESTAMP"
  ).bind(card[0], card[1], card[3], card[2], "Official MABAS Division 11 response card. Administration may replace this note with dispatch, staging, callback, or change-of-quarters instructions.", "Use this editable response-plan area for the Still, Full Still, Box, 2nd, 3rd, 4th, and interdivisional assignments shown on the attached original card.\n\nThe original approved source remains attached below for verification.", card[4], card[5], card[6])));
  await db.prepare("UPDATE box_cards SET access_notes = 'Official MABAS Division 11 response card. Administration may replace this note with dispatch, staging, callback, or change-of-quarters instructions.', details = 'Use this editable response-plan area for the Still, Full Still, Box, 2nd, 3rd, 4th, and interdivisional assignments shown on the attached original card.\n\nThe original approved source remains attached below for verification.' WHERE id LIKE 'sfd-box-%' AND details LIKE 'Open the original Stickney box card%'").run();
