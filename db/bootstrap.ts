...
export async function ensureDatabase() {
  ...
  await db.batch([
    ...
    db.prepare("CREATE TABLE IF NOT EXISTS schedule_notifications (id TEXT PRIMARY KEY NOT NULL, employee_id TEXT NOT NULL REFERENCES employees(id), title TEXT NOT NULL, message TEXT NOT NULL, in_app INTEGER NOT NULL DEFAULT 1, event_type TEXT NOT NULL DEFAULT 'general', email INTEGER NOT NULL DEFAULT 0, sms INTEGER NOT NULL DEFAULT 0, delivery_status TEXT NOT NULL DEFAULT 'queued', read_at TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, scheduled_for TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
  ...
  try { await db.prepare("ALTER TABLE schedule_notifications ADD COLUMN scheduled_for TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP").run(); } catch { /* Column already exists after migration. */ }
