...
let ready = false;
const runtimeBootstrapVersion = "stickney-runtime-bootstrap-2026-08-10-station-scheduler-v1-cad-dispatch-v1";
...
async function initializeDatabase(db: Awaited<ReturnType<typeof getDatabaseBinding>>) {
  await db.batch([
    ...
    // CAD dispatching service: apparatus/vehicle AVL, real-time incident notes,
    // partner-agency webhooks, and monitored fire-alarm panels.
    db.prepare("CREATE TABLE IF NOT EXISTS cad_units (id TEXT PRIMARY KEY NOT NULL, unit_number TEXT NOT NULL UNIQUE, name TEXT NOT NULL DEFAULT '', unit_type TEXT NOT NULL DEFAULT 'engine', agency TEXT NOT NULL DEFAULT 'Stickney', station TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'available', status_since TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, active_incident_id TEXT NOT NULL DEFAULT '', latitude REAL, longitude REAL, heading REAL, speed REAL, location_at TEXT, home_latitude REAL, home_longitude REAL, source TEXT NOT NULL DEFAULT 'manual', active INTEGER NOT NULL DEFAULT 1, created_by TEXT NOT NULL DEFAULT 'System', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
    db.prepare("CREATE INDEX IF NOT EXISTS cad_units_active_status_idx ON cad_units(active, status)"),
    db.prepare("CREATE TABLE IF NOT EXISTS cad_unit_location_history (id TEXT PRIMARY KEY NOT NULL, unit_id TEXT NOT NULL REFERENCES cad_units(id), unit_number TEXT NOT NULL DEFAULT '', latitude REAL NOT NULL, longitude REAL NOT NULL, heading REAL, speed REAL, recorded_at TEXT NOT NULL, source TEXT NOT NULL DEFAULT '')"),
    db.prepare("CREATE INDEX IF NOT EXISTS cad_unit_location_history_idx ON cad_unit_location_history(unit_id, recorded_at)"),
    db.prepare("CREATE TABLE IF NOT EXISTS cad_incident_notes (id TEXT PRIMARY KEY NOT NULL, incident_id TEXT NOT NULL, sequence INTEGER NOT NULL, note TEXT NOT NULL, category TEXT NOT NULL DEFAULT 'note', author TEXT NOT NULL DEFAULT '', source TEXT NOT NULL DEFAULT 'local', agency TEXT NOT NULL DEFAULT '', unit_number TEXT NOT NULL DEFAULT '', external_id TEXT NOT NULL DEFAULT '', event_at TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
    db.prepare("CREATE INDEX IF NOT EXISTS cad_incident_notes_incident_idx ON cad_incident_notes(incident_id, sequence)"),
    db.prepare("CREATE TABLE IF NOT EXISTS cad_agencies (id TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, agency_type TEXT NOT NULL DEFAULT 'cad', contact TEXT NOT NULL DEFAULT '', inbound_token TEXT NOT NULL DEFAULT '', outbound_url TEXT NOT NULL DEFAULT '', outbound_secret TEXT NOT NULL DEFAULT '', subscriptions TEXT NOT NULL DEFAULT '[]', active INTEGER NOT NULL DEFAULT 1, last_inbound_at TEXT, last_outbound_at TEXT, created_by TEXT NOT NULL DEFAULT 'System', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
    db.prepare("CREATE TABLE IF NOT EXISTS cad_outbound_deliveries (id TEXT PRIMARY KEY NOT NULL, agency_id TEXT NOT NULL DEFAULT '', agency_name TEXT NOT NULL DEFAULT '', event_type TEXT NOT NULL DEFAULT '', incident_id TEXT NOT NULL DEFAULT '', endpoint TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'pending', status_code INTEGER, error TEXT NOT NULL DEFAULT '', payload TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, delivered_at TEXT)"),
    db.prepare("CREATE INDEX IF NOT EXISTS cad_outbound_deliveries_idx ON cad_outbound_deliveries(agency_id, created_at)"),
    db.prepare("CREATE TABLE IF NOT EXISTS cad_alarm_panels (id TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, monitor_account TEXT NOT NULL DEFAULT '', address TEXT NOT NULL DEFAULT '', latitude REAL, longitude REAL, protocol TEXT NOT NULL DEFAULT 'webhook', inbound_token TEXT NOT NULL DEFAULT '', auto_create_incident INTEGER NOT NULL DEFAULT 1, active INTEGER NOT NULL DEFAULT 1, last_signal_at TEXT, created_by TEXT NOT NULL DEFAULT 'System', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
    db.prepare("CREATE TABLE IF NOT EXISTS cad_alarm_events (id TEXT PRIMARY KEY NOT NULL, panel_id TEXT NOT NULL REFERENCES cad_alarm_panels(id), panel_name TEXT NOT NULL DEFAULT '', external_event_id TEXT NOT NULL DEFAULT '', signal_type TEXT NOT NULL DEFAULT 'alarm', zone TEXT NOT NULL DEFAULT '', description TEXT NOT NULL DEFAULT '', priority TEXT NOT NULL DEFAULT 'high', latitude REAL, longitude REAL, incident_id TEXT NOT NULL DEFAULT '', event_at TEXT NOT NULL, received_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, raw_payload TEXT NOT NULL DEFAULT '{}', acknowledged_at TEXT, acknowledged_by TEXT NOT NULL DEFAULT '')"),
    db.prepare("CREATE INDEX IF NOT EXISTS cad_alarm_events_panel_idx ON cad_alarm_events(panel_id, received_at)"),
  ]);
