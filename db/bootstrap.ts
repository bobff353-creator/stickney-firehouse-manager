...
async function initializeDatabase(db: Awaited<ReturnType<typeof getDatabaseBinding>>) {
  await db.batch([
    ...
    db.prepare("CREATE INDEX IF NOT EXISTS field_hydrant_number_idx ON field_hydrants(hydrant_number) WHERE hydrant_number<>''"),
    db.prepare("CREATE TABLE IF NOT EXISTS road_closures (id TEXT PRIMARY KEY NOT NULL, road_name TEXT NOT NULL, reason TEXT NOT NULL DEFAULT '', path_json TEXT NOT NULL DEFAULT '[]', detour_latitude REAL NOT NULL, detour_longitude REAL NOT NULL, status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active','cleared')), started_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, expected_clear_at TEXT, created_by TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_by TEXT NOT NULL, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, cleared_by TEXT, cleared_at TEXT, clear_note TEXT NOT NULL DEFAULT '')"),
    db.prepare("CREATE INDEX IF NOT EXISTS road_closures_status_started_idx ON road_closures(status,started_at)"),
  ...
  try { await db.prepare("ALTER TABLE field_preplan_imports ADD COLUMN source_external_id TEXT NOT NULL DEFAULT ''").run(); } catch { /* Column already exists after migration. */ }
  try { await db.prepare("ALTER TABLE field_preplan_imports ADD COLUMN source_payload TEXT NOT NULL DEFAULT '{}'").run(); } catch { /* Column already exists after migration. */ }
  await db.prepare("DROP INDEX IF EXISTS field_hydrant_number_idx").run();
  await db.prepare("CREATE INDEX IF NOT EXISTS field_hydrant_number_idx ON field_hydrants(hydrant_number) WHERE hydrant_number<>''").run();
  await db.prepare("CREATE UNIQUE INDEX IF NOT EXISTS field_preplan_import_external_id_idx ON field_preplan_imports(source_external_id) WHERE source_external_id<>''").run();
  ...
  await db.prepare("INSERT OR IGNORE INTO rank_permissions(rank,permission_key,allowed) SELECT DISTINCT label,'safety_inspections.view',1 FROM pay_scales").run();
  await db.prepare("INSERT OR IGNORE INTO rank_permissions(rank,permission_key,allowed) SELECT DISTINCT label,'safety_inspections.complete',1 FROM pay_scales").run();
  await db.prepare("INSERT OR IGNORE INTO rank_permissions(rank,permission_key,allowed) SELECT DISTINCT label,'safety_inspections.manage',CASE WHEN lower(label) LIKE '%chief%' OR lower(label) LIKE '%captain%' OR lower(label) LIKE '%lieutenant%' THEN 1 ELSE 0 END FROM pay_scales").run();
