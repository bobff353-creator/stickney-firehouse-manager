...
test("station scheduler tables exist in schema and bootstrap", async () => {
  ...
  assert.equal(bootstrap.includes("safety-inspection-library-v2"), true, "bootstrap version includes the latest additive schema");
...
test("scheduler uses the scoped Stickney mobile workspace instead of prototype branding", async () => {
  ...
  assert.equal(styles.includes("button.calendar-has-shift"), true, "the visible shift keeps its saved color across the day card");
...
test("calendar day view manages one-day openings and assignments without changing shift patterns", async () => {
  ...
  assert.equal(component.includes('value={selected}'), true, "admin stages a daily assignment before saving");
  assert.equal(component.includes('Save assignment'), true);
