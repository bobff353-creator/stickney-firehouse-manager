...
test("scheduler uses the scoped Stickney mobile workspace instead of prototype branding", async () => {
  ...
  assert.equal(component.includes("calendarTimeBlocks.map"), true, "every date renders the same ordered morning, afternoon, and overnight sequence");
  assert.equal(component.includes("calendar-shift-sequence"), true, "calendar shows all three daily time bands at once");
  assert.equal(component.includes("12_000"), false, "daily shift bands no longer disappear behind automatic rotation");
  assert.equal(component.includes("Pause rotation"), false, "the obsolete rotation control is removed");
  ...
  assert.equal(styles.includes(".calendar-time-band.scheduled"), true, "each saved shift keeps its own color inside the day card");
