...
import {
  ...
  hasOccupancyProfile,
  occupancyProfile,
} from "../app/preplans/profiles.ts";

const route = fs.readFileSync(
  new URL("../app/api/field-preplans/operational/route.ts", import.meta.url),
  "utf8",
);
const panel = fs.readFileSync(
  new URL("../app/preplans/operational-panel.tsx", import.meta.url),
  "utf8",
);
const respondRoute = fs.readFileSync(
...
test("construction profiles preserve unknown facts and bound counts", () => {
  assert.deepEqual(
    ...
    }),
    {
      ...
      roofType: "",
      roofSupportSystem: "",
      lightweightConstruction: "unknown",
      bowstringTruss: "unknown",
      basementType: "",
      floorsAboveGrade: 2,
      floorsBelowGrade: null,
      fortifiedAccess: "unknown",
      notes: "",
    },
  );
  assert.equal(hasConstructionProfile(constructionProfile({})), false);
  assert.equal(
    hasConstructionProfile(constructionProfile({ bowstringTruss: "yes" })),
    true,
  );
});

test("occupancy profiles distinguish verified zero from missing information", () => {
  const profile = occupancyProfile({
    classification: "School",
    daytimeOccupancy: 0,
    peakOccupancy: "850",
    sleepingOccupants: "no",
  });
...
test("Field and Respond persist and consume the same structured profiles", () => {
  ...
  assert.match(
    panel,
    /No verified structured construction profile is recorded/,
  );
  assert.match(respondRoute, /construction:\s*constructionProfile/);
  assert.match(respondRoute, /occupancy:\s*occupancyProfile/);
