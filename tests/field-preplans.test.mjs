...
test("Field Preplans provides map-first quick and detailed capture", async () => {
  ...
  assert.match(page, /Advanced details & publication/);
  ...
  assert.match(page, /Show on preplan map/);
  assert.match(page, /No mapped systems have been added/);
  assert.match(page, /recordMode==="view"/);
  ...
  assert.match(styles, /\.preplan-focus-map-panel\{grid-template-columns:minmax\(0,1fr\)\}/);
  assert.match(styles, /\.preplan-focus-map-panel>\*\{min-width:0\}/);
  ...
  assert.match(styles, /\.mapped-feature-list \.mapped-feature-record/);
