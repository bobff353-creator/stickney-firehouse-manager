...
test('distribution requires valid dates and includes only open positions inside both boundaries', async () => {
  ...
  const pg = new PGlite();
  try {
    await pg.exec(`CREATE TABLE station_shift_types(id text,start_time text,end_time text,active int DEFAULT 1,anchor_date text DEFAULT '',repeat_every_days int DEFAULT 0);
      CREATE TABLE station_schedule_entries(id text,entry_date text,shift_type_id text);
      CREATE TABLE station_shift_slots(id text,entry_id text,role text,status text,start_time text,end_time text,sort_order int,is_extra int DEFAULT 0,employee_id text);
      INSERT INTO station_shift_types(id,start_time,end_time) VALUES('day','06:00','12:00');
      ...
      INSERT INTO station_shift_slots(id,entry_id,role,status,start_time,end_time,sort_order) SELECT id,id,'FF/Attendant','open','','',0 FROM station_schedule_entries;
