...
createServer(async (req, res) => {
  ...
  if (url.pathname !== '/frame') {
    const width = Number(url.searchParams.get('width')) === 1920 ? 1920 : Number(url.searchParams.get('width')) === 1213 ? 1213 : 1280;
    const height = width === 1920 ? 1080 : width === 1213 ? 666 : 720;
  ...
  <div class="board-summary"><article><strong>Preview only</strong></article></div><div class="board-grid redesigned"><section class="board-panel chief-board-panel"><h2>Content region</h2></section><section class="board-panel rotating-panel news"><header><h2>Firefighter Close Calls</h2><span>Preview only</span></header><div class="rotation-content"><div class="rotation-slide"><div class="close-call-list">${['STRAY BULLET PIERCES ROOF OF FIREHOUSE – TEXAS.', 'PREVIEW LONG HEADLINE: FIREFIGHTERS REVIEW A TRAINING INCIDENT AND DISCUSS LESSONS FOR FUTURE EMERGENCY RESPONSES', 'PREVIEW REPORT THREE — READ THE FULL REPORT'].map(title => `<a href="#"><time><span>Posted</span>Sep 7, 2026</time><div><span class="close-call-kicker">Incident report</span><strong>${title}</strong><p>Preview summary text for checking that the normal portal retains summaries while TV mode preserves space for headlines and report links.</p><small>Read the complete report <b>↗</b></small></div></a>`).join('')}</div></div></div></section></div><footer>Preview footer</footer></section></main>
  <script>addEventListener('load',()=>{const cards=[...document.querySelectorAll('.close-call-list a')];const fits=cards.every(card=>{const outer=card.getBoundingClientRect();return [...card.querySelectorAll('strong,small,time')].every(n=>{const r=n.getBoundingClientRect();return r.top>=outer.top&&r.bottom<=outer.bottom&&r.right<=outer.right;});});const result=document.createElement('p');result.textContent='Close-call content bounds: '+(fits?'PASS':'FAIL');document.querySelector('footer').append(result);});</script>
