...
export default function SmartAlerts({ icon, onNavigate, onReminderSettings }: { icon: ReactNode; onNavigate: (page: string) => void; onReminderSettings?: () => void }) {
  ...
  return <div className="smart-alerts" ref={root}><button className="notification-button" aria-label={error ? "Smart alerts unavailable" : loaded ? `${alerts.length} smart alerts` : "Loading smart alerts"} title="Notifications and push settings" aria-expanded={open} onClick={() => { setOpen((value) => !value); if (!open) void load(); }}>{icon}{error && <span>!</span>}{!error && alerts.length > 0 && <span>{alerts.length > 99 ? "99+" : alerts.length}</span>}</button>{open && <section className="alert-popover"><header><div><strong>Smart alerts</strong><small>Operational and administrative attention</small></div><button disabled={refreshing} onClick={() => void load()}>{refreshing ? "Refreshing…" : "Refresh"}</button></header><section className="alert-push-settings"><h3>Push notification settings</h3><PushNotifications />{onReminderSettings && <button type="button" className="quiet-button" onClick={() => { onReminderSettings(); setOpen(false); }}>Department reminder settings →</button>}</section>{error ? <p className="alert-error" role="alert">{error}</p> : !loaded ? <p role="status">Checking current alerts…</p> : alerts.length ? <div className="alert-list">{alerts.map((alert) => <button key={alert.id} className={alert.severity} onClick={() => { onNavigate(alert.page); setOpen(false); }}><i>{alert.severity === "critical" ? "!" : alert.severity === "warning" ? "△" : "i"}</i><span><small>{alert.category}</small><strong>{alert.title}</strong><p>{alert.detail}</p></span><b>›</b></button>)}</div> : <div className="alerts-clear"><span>✓</span><strong>No active alerts</strong><p>Current records do not require attention.</p></div>}<footer><span>Smart alerts refresh every minute</span></footer></section>}</div>;
}
