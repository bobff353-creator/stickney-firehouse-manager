...
async function access(request:Request,db:Db){
  ...
  const set=new Set(rankRows.results.length?rankRows.results.filter((item)=>item.allowed).map((item)=>item.permissionKey):defaultPermissionsForRank(row!.rank));for(const item of overrides.results)item.effect==="allow"?set.add(item.permissionKey):set.delete(item.permissionKey);
...
export async function POST(request:Request){
  try{const db=await ensureDatabase(),auth=await access(request,db);if(!auth.edit)return Response.json({error:"Field hydrant edit permission is required."},{status:403});const body=await request.json() as Record<string,unknown>,action=txt(body.action,40);
    if(action==="deleteHydrant"){const id=txt(body.id,80),confirmation=txt(body.confirmation,20);if(!id||confirmation!=="DELETE")return Response.json({error:"Confirm Delete is required before a hydrant can be removed."},{status:400});
      const removed=await db.prepare("WITH deleted_tests AS (DELETE FROM field_hydrant_flow_tests WHERE test_hydrant_id=? OR flow_hydrant_id=?), deleted_flushes AS (DELETE FROM field_hydrant_flushes WHERE hydrant_id=?) DELETE FROM field_hydrants WHERE id=? RETURNING id").bind(id,id,id,id).all<{id:string}>();
