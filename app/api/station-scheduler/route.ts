...
export const STATION_ROLES = ["Officer/AO", "Engine Driver", "Ambulance Driver", "FF/Attendant"] as const;
...
export async function GET(request: Request) {
  try {
    ...
    const [shiftTypes, shiftTypeRoles, entries, slots, standing, trades, claims, timeOff, timeOffDates, reminderRules, timing, weights, interest, offers] = await Promise.all([
      ...
      db.prepare("SELECT id,entry_date entryDate,shift_type_id shiftTypeId FROM station_schedule_entries WHERE entry_date>=date(?, '-45 day') ORDER BY entry_date").bind(today).all(),
      db.prepare("SELECT s.id,s.entry_id entryId,s.role,s.employee_id employeeId,e.name employeeName,s.status,s.sort_order sortOrder,en.entry_date entryDate,en.shift_type_id shiftTypeId FROM station_shift_slots s JOIN station_schedule_entries en ON en.id=s.entry_id LEFT JOIN employees e ON e.id=s.employee_id WHERE en.entry_date>=date(?, '-45 day') ORDER BY en.entry_date,s.sort_order").bind(today).all(),
      db.prepare("SELECT sa.id,sa.employee_id employeeId,e.name employeeName,sa.shift_type_id shiftTypeId,sa.role,sa.active FROM station_standing_assignments sa JOIN employees e ON e.id=sa.employee_id WHERE sa.active=1 ORDER BY e.name COLLATE NOCASE").all(),
...
async function createEntry(db: Db, current: Viewer, payload: Record<string, unknown>, requireAdmin: () => void) {
  ...
  const standing = await db.prepare("SELECT employee_id employeeId,role FROM station_standing_assignments WHERE shift_type_id=? AND active=1").bind(shiftTypeId).all<{ employeeId: string; role: string }>();
...
async function clearSlot(db: Db, payload: Record<string, unknown>, requireAdmin: () => void) {
  requireAdmin();
  await db.prepare("UPDATE station_shift_slots SET employee_id=NULL,status='open' WHERE id=?").bind(String(payload.slotId ?? "")).run();
  return ok();
}
...
async function runAutoDistribution(db: Db, payload: Record<string, unknown>, requireAdmin: () => void) {
  ...
  const openRows = (await db.prepare("SELECT s.id,s.role,en.entry_date entryDate,t.start_time startTime,t.end_time endTime FROM station_shift_slots s JOIN station_schedule_entries en ON en.id=s.entry_id JOIN station_shift_types t ON t.id=en.shift_type_id WHERE s.status='open' AND en.entry_date>=? ORDER BY en.entry_date,s.sort_order").bind(fromDate).all<{ id: string; role: string; entryDate: string; startTime: string; endTime: string }>()).results;
...
async function submitClaim(db: Db, current: Viewer, payload: Record<string, unknown>) {
  ...
  const emp = await db.prepare("SELECT p.label rank,COALESCE(ep.station_roles,'[]') roles,COALESCE(ep.acting_officer_eligible,0) actingOfficerEligible FROM employees e JOIN pay_scales p ON p.id=e.pay_scale_id LEFT JOIN employee_profiles ep ON ep.employee_id=e.id WHERE e.id=? AND e.active=1").bind(employeeId).first<{ rank: string; roles: string; actingOfficerEligible: number }>();
...
async function submitTimeOff(db: Db, current: Viewer, payload: Record<string, unknown>) {
  ...
  const approver = await db.prepare("SELECT p.label rank,COALESCE(ep.station_roles,'[]') roles,COALESCE(ep.acting_officer_eligible,0) actingOfficerEligible FROM employees e JOIN pay_scales p ON p.id=e.pay_scale_id LEFT JOIN employee_profiles ep ON ep.employee_id=e.id WHERE e.id=? AND e.active=1").bind(approverEmployeeId).first<{ rank: string; roles: string; actingOfficerEligible: number }>();
