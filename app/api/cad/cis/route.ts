...
async function isAdmin(request: Request, db: Awaited<ReturnType<typeof ensureDatabase>>) {
  ...
  const profile = email ? await db.prepare("SELECT is_admin AS isAdmin FROM employee_profiles WHERE lower(email) = ? LIMIT 1").bind(email).first<{ isAdmin: number }>() : null;
...
async function authenticated(request: Request, body: string, secret: string) {
  ...
  if (authorization.startsWith("Bearer ")) {
    return safeEqual(new TextEncoder().encode(authorization.slice(7)), new TextEncoder().encode(secret));
  }
  const signature = (request.headers.get("x-cis-signature") ?? "").replace(/^sha256=/i, "");
  if (!/^[a-f0-9]{64}$/i.test(signature)) return false;
  const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  const digest = new Uint8Array(await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(body)));
  const provided = Uint8Array.from(signature.match(/.{2}/g) ?? [], (pair) => Number.parseInt(pair, 16));
...
export async function GET(request: Request) {
  try {
    ...
    const accepted = await db.prepare("SELECT COUNT(*) AS count FROM cad_inbound_receipts WHERE provider = 'cis' AND status = 'accepted'").first<{ count: number }>();
    const failed = await db.prepare("SELECT COUNT(*) AS count FROM cad_inbound_receipts WHERE provider = 'cis' AND status = 'rejected'").first<{ count: number }>();
    return Response.json({
      provider: "CIS / Cicero Consolidated Dispatch",
      stage: env.CIS_CAD_WEBHOOK_SECRET ? "credentials_configured" : "prebuilt",
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Unable to load CIS CAD status." }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  try {
    const db = await ensureDatabase();
    if (!await isAdmin(request, db)) return Response.json({ error: "Administrator access is required." }, { status: 403 });
    const payload = await request.json() as { sample?: unknown; contentType?: unknown };
    const sample = typeof payload.sample === "string" ? payload.sample : JSON.stringify(payload.sample ?? {});
    if (sample.length > 262_144) return Response.json({ error: "Sample is larger than 256 KB." }, { status: 413 });
    const result = parseCisCadPayload(sample, String(payload.contentType || "application/json"));
    return Response.json(result, { status: result.ok ? 200 : 422, headers: { "cache-control": "no-store" } });
  } catch {
    return Response.json({ error: "The sample could not be validated." }, { status: 400 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (declaredLength > 262_144) return Response.json({ error: "Payload is larger than 256 KB.", receiptId }, { status: 413 });
    body = await request.text();
    if (!body || body.length > 262_144) return Response.json({ error: "Payload must be between 1 byte and 256 KB.", receiptId }, { status: 413 });

    const env = await runtime();
    if (!env.CIS_CAD_WEBHOOK_SECRET) return Response.json({ error: "CIS CAD delivery is not configured.", receiptId }, { status: 503 });
    if (!await authenticated(request, body, env.CIS_CAD_WEBHOOK_SECRET)) return Response.json({ error: "Invalid CIS CAD authentication.", receiptId }, { status: 401 });

    const db = await ensureDatabase();
    const payloadHash = await sha256(body);
    const prior = await db.prepare("SELECT id FROM cad_inbound_receipts WHERE provider = 'cis' AND dedupe_key = ? LIMIT 1").bind(payloadHash).first<{ id: string }>();
    if (prior) return Response.json({ accepted: true, duplicate: true, receiptId: prior.id });
    ...
    if (!parsed.ok) {
      ...
      ).bind(receiptId, payloadHash, parsed.format, body, parsed.error).run();
      return Response.json({ accepted: false, receiptId, error: parsed.error }, { status: 422 });
    }

    const incident = parsed.incident;
    const active = incident.eventType === "close" ? 0 : 1;
    const timeOut = incident.timeOut || chicagoMilitaryTime(incident.dispatchedAt);
    ...
    ).bind(receiptId, payloadHash, incident.eventId, incident.incidentId, incident.eventType, parsed.format, body, JSON.stringify(incident));
    ...
    ).bind(incident.incidentId, `cis-${incident.incidentId}`, incident.callType, incident.category, incident.address, incident.city, incident.narrative, incident.respondingUnits, incident.longitude, incident.latitude, incident.dispatchedAt, timeOut, JSON.stringify(incident), active, active);
    ...
    return Response.json({ accepted: true, duplicate: false, receiptId, incidentId: incident.incidentId, eventType: incident.eventType });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Unable to process CIS CAD delivery.", receiptId }, { status: 500 });
  }
}
