...
export async function GET(request: Request) {
  ...
  try {
    ...
    checks.push({
      ...
      state: "healthy",
      value: "Online",
      detail: `${bucketsResult.data.length} storage bucket${bucketsResult.data.length === 1 ? "" : "s"} reachable. Usage and capacity require provider telemetry.`,
      verifiedAt: checkedAt,
    });
  } catch {
  ...
  checks.push({
    ...
    detail: "This is the application version serving the current request.",
    verifiedAt: checkedAt,
  });
  ...
  checks.push(
    ...
    unavailable("database-usage", "Database used / capacity", "Provider usage telemetry is not available to the portal runtime.", checkedAt),
    unavailable("storage-usage", "Storage used / capacity", "Provider usage telemetry is not available to the portal runtime.", checkedAt),
