...
export async function GET(request: Request) {
  try {
    ...
    if (isAdmin) { const expiration = await evaluatePreplanExpirations(db); if (expiration.reviewCount) alerts.push({ id: "preplan-expiration-review", severity: expiration.expired.length || expiration.invalid.length ? "critical" : "warning", category: "Preplans", title: "Operational preplan review required", detail: `${expiration.expired.length} expired · ${expiration.upcoming.length} expiring within 30 days · ${expiration.invalid.length} invalid expiration date${expiration.invalid.length === 1 ? "" : "s"}.`, page: "Field Preplans" }); }
