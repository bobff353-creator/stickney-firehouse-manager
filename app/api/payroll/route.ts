...
export async function POST(request: Request) {
  try {
    ...
    if (action === "saveEmployee") {
      ...
      await db.prepare("INSERT INTO employee_profiles (employee_id, employee_number, start_date, end_date, date_of_birth, phone, email, address_line_1, city, state, postal_code, employment_type, emergency_name, emergency_relationship, emergency_phone, notes, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(employee_id) DO UPDATE SET employee_number = excluded.employee_number, start_date = excluded.start_date, end_date = excluded.end_date, date_of_birth = excluded.date_of_birth, phone = excluded.phone, email = excluded.email, address_line_1 = excluded.address_line_1, city = excluded.city, state = excluded.state, postal_code = excluded.postal_code, employment_type = excluded.employment_type, emergency_name = excluded.emergency_name, emergency_relationship = excluded.emergency_relationship, emergency_phone = excluded.emergency_phone, notes = excluded.notes, updated_at = CURRENT_TIMESTAMP").bind(id, String(payload.employeeNumber ?? "").trim() || null, String(payload.startDate ?? "") || null, String(payload.endDate ?? "") || null, String(payload.dateOfBirth ?? "") || null, String(payload.phone ?? "").trim() || null, String(payload.email ?? "").trim() || null, String(payload.addressLine1 ?? "").trim() || null, String(payload.city ?? "").trim() || null, String(payload.state ?? "").trim() || null, String(payload.postalCode ?? "").trim() || null, String(payload.employmentType ?? "Part-time"), String(payload.emergencyName ?? "").trim() || null, String(payload.emergencyRelationship ?? "").trim() || null, String(payload.emergencyPhone ?? "").trim() || null, String(payload.notes ?? "").trim() || null).run();
