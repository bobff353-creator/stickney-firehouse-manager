...
export async function GET(request: Request) {
  try {
    ...
    const [staffing, approvals, calls, dispatchCalls, log, payrollWaiting, newMembers] = await Promise.all([
      ...
      db.prepare("SELECT report_number AS reportNumber, time_out AS timeOut, time_in AS timeIn, responding_units AS respondingUnits, address, call_type AS callType FROM daily_log_calls WHERE log_date = ? AND NOT EXISTS (SELECT 1 FROM dispatch_incidents WHERE trim(dispatch_incidents.incident_id)=trim(daily_log_calls.report_number)) ORDER BY sort_order DESC").bind(now.date).all(),
      db.prepare("SELECT incident_id AS reportNumber, time_out AS timeOut, '' AS timeIn, responding_units AS respondingUnits, address, call_type AS callType, narrative, source_system AS source, dispatched_at AS dispatchedAt FROM dispatch_incidents WHERE active = 1 AND cleared_at IS NULL AND (source_system='CIS CAD' OR datetime(dispatched_at) >= datetime('now', '-12 hours')) AND NOT EXISTS (SELECT 1 FROM daily_log_calls WHERE trim(daily_log_calls.report_number) = trim(dispatch_incidents.incident_id) AND trim(daily_log_calls.time_in) <> '') ORDER BY datetime(dispatched_at) DESC").all(),
