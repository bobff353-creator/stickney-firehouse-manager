...
async function bridgeActiveCalls() {
  try {
    const { env } = await import("cloudflare:workers");
    const token = (env as unknown as RuntimeEnv).DISPATCH_BRIDGE_READ_TOKEN;
    if (!token) return [];
    const response = await fetch("https://stickney-dispatch-bridge.bobff353.chatgpt.site/api/active-calls", {
      headers: { authorization: `Bearer ${token}` },
    });
    if (!response.ok) return [];
    const payload = await response.json() as { activeCalls?: Array<Record<string, unknown>> };
    return Array.isArray(payload.activeCalls) ? payload.activeCalls : [];
  } catch {
    return [];
  }
}
...
export async function GET() {
  try {
    ...
    const [staffing, approvals, calls, dispatchCalls, bridgedDispatchCalls, log, payrollWaiting, newMembers] = await Promise.all([
      ...
      db.prepare("SELECT incident_id AS reportNumber, time_out AS timeOut, '' AS timeIn, responding_units AS respondingUnits, address, call_type AS callType, narrative, 'CAD email' AS source FROM dispatch_incidents WHERE active = 1 AND cleared_at IS NULL AND datetime(dispatched_at) >= datetime('now', '-12 hours') ORDER BY datetime(dispatched_at) DESC LIMIT 12").all(),
      bridgeActiveCalls(),
    ...
    const bridgedReportNumbers = new Set(bridgedDispatchCalls.map((call) => String(call.reportNumber || "")));
    const emailActiveCalls = [...bridgedDispatchCalls, ...localEmailActiveCalls.filter((call) => !bridgedReportNumbers.has(String(call.reportNumber || "")))];
    const manualReportNumbers = new Set(manualActiveCalls.map((call) => String(call.reportNumber || "")));
    const activeCalls = [...emailActiveCalls.filter((call) => !manualReportNumbers.has(String(call.reportNumber || ""))), ...manualActiveCalls];
