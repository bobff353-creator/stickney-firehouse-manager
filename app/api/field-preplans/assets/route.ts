...
export async function POST(request:Request){
  ...
  try{
    ...
    const preplanId=String(form.get("preplanId")??"").trim(),levelId=String(form.get("levelId")??"").trim();
    ...
    await db.prepare("INSERT INTO field_preplan_assets(id,preplan_id,level_id,category,original_filename,object_key,mime_type,file_size,caption,created_by,updated_by) VALUES(?,?,NULLIF(?,''),?,?,?,?,?,?,?,?)").bind(id,preplanId,levelId,assetType,safeName,objectKey,file.type,file.size,caption,user,user).run();
