...
import { isSafePhoto } from "../../../upload-security";
...
export async function POST(request:Request) {
  ...
  try {
    ...
    if (!(file instanceof File) || file.size > 12 * 1024 * 1024 || !await isSafePhoto(file)) return Response.json({ error:"Choose a valid JPG, PNG, WebP, GIF, HEIC, HEIF, or AVIF photo up to 12 MB. SVG and executable content are not supported." }, { status:400 });
    ...
    if (storedKey) try { await (await runtime()).BUCKET?.delete(storedKey); } catch { /* Best effort cleanup. */ }
    return Response.json({ error:"Unable to upload photo. Please retry." }, { status:500 });
  }
}
