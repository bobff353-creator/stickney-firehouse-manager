...
export async function GET(request: Request) {
  try {
    ...
    const [staffing, calls, callTiming, approvals, entries, settings] = await Promise.all([
      ...
      db.prepare("SELECT t.employee_id AS employeeId, t.period_start AS periodStart, t.work_date AS date, t.category, t.hours, ps.regular_rate AS regularRate, ps.holiday_rate AS holidayRate FROM time_entries t JOIN employees e ON e.id = t.employee_id JOIN pay_scales ps ON ps.id = e.pay_scale_id WHERE date(t.work_date) >= date('now', '-365 day') ORDER BY t.employee_id, t.period_start, t.work_date, t.category").all(),
    ...
    for (const row of staffing.results as Array<Record<string, unknown>>) metric(String(row.date)).staffingGaps += Math.max(0, 4 - Number(row.filled || 0));
