...
export async function GET(request: Request) {
  try {
    ...
    const [staffing, calls, callTiming, approvals, entries, settings, rateHistory] = await Promise.all([
      ...
      db.prepare("SELECT t.employee_id AS employeeId, t.period_start AS periodStart, t.work_date AS date, t.category, t.hours, ps.label AS rank, ps.id AS payScaleId, ps.regular_rate AS regularRate, ps.overtime_rate AS overtimeRate, ps.holiday_rate AS holidayRate, COALESCE(ep.is_dpw,0) AS isDpw FROM time_entries t JOIN employees e ON e.id = t.employee_id JOIN pay_scales ps ON ps.id = e.pay_scale_id LEFT JOIN employee_profiles ep ON ep.employee_id = e.id WHERE date(t.work_date) >= date('now', '-365 day') ORDER BY t.employee_id, t.period_start, t.work_date, t.category").all(),
      ...
      db.prepare("SELECT pay_scale_id AS payScaleId, effective_date AS effectiveDate, regular_rate AS regularRate, overtime_rate AS overtimeRate, holiday_rate AS holidayRate FROM pay_rate_history ORDER BY pay_scale_id, effective_date DESC").all(),
    ]);
    ...
    if (!settings) throw new Error("Payroll settings are missing. Configure them before calculating costs.");
    const payrollDetails = buildPayrollDetails(entries.results as never[], rateHistory.results as never[], settings);
