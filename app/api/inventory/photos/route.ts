...
export async function POST(request:Request){
  ...
  const {env}=await import("cloudflare:workers"); if(!env.BUCKET)return Response.json({error:"Department photo storage is unavailable."},{status:503});
...
export async function GET(request:Request){
  ...
  const {env}=await import("cloudflare:workers"),object=await env.BUCKET?.get(photo.objectKey);if(!object)return new Response("Photo unavailable",{status:404});return new Response(object.body,{headers:{"Content-Type":photo.contentType,"Cache-Control":"private, max-age=300"}});
}
...
export async function DELETE(request:Request){
  ...
  const {env}=await import("cloudflare:workers");await env.BUCKET?.delete(photo.objectKey);await db.prepare("DELETE FROM inventory_photos WHERE id=?").bind(id).run();return Response.json({deleted:true});
}
