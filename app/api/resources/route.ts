...
export async function GET(request: Request) {
  try {
    ...
    const rows = type === "policy"
      ...
      : await db.prepare("SELECT id, title, address, box_number AS boxNumber, access_notes AS accessNotes, details, department, document_url AS documentUrl, document_page AS documentPage, effective_date AS effectiveDate, review_date AS reviewDate, status, created_by AS createdBy, COALESCE(created_at, updated_at) AS createdAt, updated_by AS updatedBy, updated_at AS updatedAt FROM box_cards ORDER BY department COLLATE NOCASE, title COLLATE NOCASE").all();
...
export async function POST(request: Request) {
  try {
    ...
    if (type === "policy") {
      ...
      await db.prepare("INSERT INTO box_cards (id, title, address, box_number, access_notes, details, department, document_url, document_page, effective_date, review_date, created_by, updated_by, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET title = excluded.title, address = excluded.address, box_number = excluded.box_number, access_notes = excluded.access_notes, details = excluded.details, department = excluded.department, document_url = excluded.document_url, document_page = excluded.document_page, effective_date = excluded.effective_date, review_date = excluded.review_date, updated_by = excluded.updated_by, updated_at = CURRENT_TIMESTAMP").bind(id, title, String(body.address ?? "").trim(), String(body.boxNumber ?? "").trim(), String(body.accessNotes ?? "").trim(), String(body.details ?? "").trim(), String(body.department ?? "Stickney").trim() || "Stickney", String(body.documentUrl ?? "").trim(), Number(body.documentPage ?? 0), String(body.effectiveDate ?? ""), String(body.reviewDate ?? ""), updatedBy, updatedBy).run();
    }
