...
const sourceFile = "FlowMSP Preplan Report 09_01_2026.csv";
const text = (value:unknown, limit=2000) => String(value ?? "").trim().slice(0, limit);
const number = (value:unknown) => { const result=Number(value); return Number.isFinite(result)?result:Number.NaN; };
...
function hydrantNote(item:HydrantImport,flowId:string) {
  const details=[
    "Imported from FlowMSP on 2026-09-01.",
    `FlowMSP source ID: ${flowId}.`,
    "GPS coordinates are authoritative; the displayed address is provider-generated.",
  ];
...
export async function POST(request:Request) {
  try {
    ...
    let hydrantsImported=0,hydrantsRejected=0;
    for(let index=0;index<hydrants.length;index+=40){
      ...
      for(const item of hydrants.slice(index,index+40)){
        ...
        statements.push(db.prepare("INSERT INTO field_hydrants(id,hydrant_number,address,latitude,longitude,service_status,manufacturer,model,port_count,port_sizes,notes,created_by,updated_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET hydrant_number=excluded.hydrant_number,address=excluded.address,latitude=excluded.latitude,longitude=excluded.longitude,service_status=excluded.service_status,notes=CASE WHEN field_hydrants.notes='' OR field_hydrants.notes LIKE 'Imported from FlowMSP%' THEN excluded.notes ELSE field_hydrants.notes END,updated_by=excluded.updated_by,updated_at=CURRENT_TIMESTAMP")
    ...
    let preplansQueued=0,preplansLinked=0,preplansMerged=0,preplansRejected=0;
    for(const item of preplans){
      ...
      const category=text(item.matchCategory,100),candidateId=text(item.portalRecordId,100),payload=text(JSON.stringify(item),20000);
      if(category==="Strong live preplan candidate"&&candidateId){
      ...
      if(category==="Strong import-queue candidate"&&candidateId){
    ...
    return Response.json({ok:true,hydrantsImported,hydrantsRejected,preplansQueued,preplansLinked,preplansMerged,preplansRejected});
  } catch(error) {
