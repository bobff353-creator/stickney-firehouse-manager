...
export async function readBoardLinksRow(db: Database) {
  ...
  return { value: row.value, settings: { revision: data.revision, updatedAt: data.updatedAt, ...(typeof data.trainingRevision === 'string' ? { trainingRevision: data.trainingRevision } : {}), sections } };
}
export async function readBoardLinks(db: Database) { return (await readBoardLinksRow(db)).settings; }
export async function saveBoardLinks(db: Database, id: BoardLinkSectionId, section: BoardLinkSection, expectedRevision: string, actor: string) {
  const current = await readBoardLinksRow(db);
  ...
  const settings: BoardLinks = { ...current.settings, revision: crypto.randomUUID(), updatedAt: new Date().toISOString(), sections: { ...current.settings.sections, [id]: section } };
