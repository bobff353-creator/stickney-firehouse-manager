...
export default function DailyLog({ employees, onPayrollSynced }: { employees: LogEmployee[]; onPayrollSynced?: () => void }) {
  ...
  return <section className={`logbook-page ${readOnly ? "is-locked" : "is-editable"}`}>
    ...
    <fieldset className="logbook-fields" disabled={readOnly}>
      ...
      <article className="content-card calls-card"><div className="section-header"><div><h2>Calls & Responses</h2><p>Times use four-digit military format. Tap Now for Time Out or Done to record the current Time In.</p></div><button className="add-call" onClick={() => { setCalls((current) => [...current, blankCall()]); markDirty(); }}>＋ Add Call</button></div><datalist id="known-addresses">{addresses.map((address) => <option key={address} value={address} />)}</datalist><div className="call-list">{calls.map((call, index) => <div className="call-row" key={call.id}><div className="call-number">{index + 1}</div><label className="call-report"><span>Report #</span><input value={call.reportNumber} onChange={(event) => updateCall(call.id, { reportNumber: event.target.value })} /></label><label className="call-time-out"><span>Time out (military)</span><div className="time-now"><input type="text" inputMode="numeric" maxLength={4} pattern="(?:[01]\d|2[0-3])[0-5]\d" placeholder="0000" aria-label="Time out in four-digit military time" value={formatMilitaryTime(call.timeOut)} onChange={(event) => updateCall(call.id, { timeOut: event.target.value.replace(/\D/g, "").slice(0, 4) })} onBlur={(event) => finishMilitaryTime(call.id, "timeOut", event.target.value)} /><button onClick={() => updateCall(call.id, { timeOut: nowTime() })}>Now</button></div></label><label className="call-time-in"><span>Time in (military)</span><div className="time-now"><input type="text" inputMode="numeric" maxLength={4} pattern="(?:[01]\d|2[0-3])[0-5]\d" placeholder="0000" aria-label="Time in in four-digit military time" value={formatMilitaryTime(call.timeIn)} onChange={(event) => updateCall(call.id, { timeIn: event.target.value.replace(/\D/g, "").slice(0, 4) })} onBlur={(event) => finishMilitaryTime(call.id, "timeIn", event.target.value)} /><button title="Set Time In to now and close this active call" onClick={() => updateCall(call.id, { timeIn: nowTime() })}>Done</button></div></label><label className="call-units"><span>Responding units</span><input placeholder="1203, 1205…" value={call.respondingUnits} onChange={(event) => updateCall(call.id, { respondingUnits: event.target.value })} /></label><label className="call-address"><span>Address</span><input list="known-addresses" autoComplete="street-address" placeholder="Start typing an address…" value={call.address} onChange={(event) => updateCall(call.id, { address: event.target.value })} /></label><label className="call-type"><span>Type</span><select value={call.callType} onChange={(event) => updateCall(call.id, { callType: event.target.value })}>{call.callType && !callTypes.includes(call.callType) && <option>{call.callType}</option>}{callTypes.map((type) => <option key={type}>{type}</option>)}</select></label>{calls.length > 1 && <button className="remove-call" aria-label={`Remove call ${index + 1}`} onClick={() => { setCalls((current) => current.filter((item) => item.id !== call.id)); markDirty(); }}>×</button>}</div>)}</div><button className="add-call bottom" onClick={() => { setCalls((current) => [...current, blankCall()]); markDirty(); }}>＋ Add Another Call</button></article>
      <article className="content-card apparatus-check-log"><div className="section-header"><div><h2>Fleet &amp; Inventory Checks</h2><p>Completed apparatus inspections are recorded automatically from Fleet.</p></div><b>{apparatusChecks.length} completed</b></div>{apparatusChecks.length ? <div>{apparatusChecks.map((check) => <a key={check.id} href={`/inventory?apparatus=${encodeURIComponent(check.apparatusId)}&check=${encodeURIComponent(check.checkType)}`}><span><strong>{check.unit} · {check.checkType.replaceAll("_", " ")} check</strong><small>Completed {new Date(check.completedAt).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })} by {check.completedBy}</small></span><b className={check.failedItems ? "has-failures" : "clear"}>{check.failedItems ? `${check.failedItems} failed item${check.failedItems === 1 ? "" : "s"}` : "All items recorded"}</b></a>)}</div> : <p className="apparatus-check-empty">No Fleet or Inventory checks were completed on this log date.</p>}</article>
