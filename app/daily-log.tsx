...
export default function DailyLog({ employees, onPayrollSynced }: { employees: LogEmployee[]; onPayrollSynced?: () => void }) {
  ...
  return <section className={`logbook-page ${readOnly ? "is-locked" : "is-editable"}`}>
    ...
    <fieldset className="logbook-fields" disabled={readOnly}>
      ...
      <article className="content-card calls-card"><div className="section-header"><div><h2>Calls & Responses</h2><p>Tap Now for the current time, then adjust it if needed.</p></div><button className="add-call" onClick={() => { setCalls((current) => [...current, blankCall()]); markDirty(); }}>＋ Add Call</button></div><datalist id="known-addresses">{addresses.map((address) => <option key={address} value={address} />)}</datalist><div className="call-list">{calls.map((call, index) => <div className="call-row" key={call.id}><div className="call-number">{index + 1}</div><label className="call-report"><span>Report #</span><input value={call.reportNumber} onChange={(event) => updateCall(call.id, { reportNumber: event.target.value })} /></label><label className="call-time-out"><span>Time out</span><div className="time-now"><input type="time" step="60" value={call.timeOut} onChange={(event) => updateCall(call.id, { timeOut: event.target.value })} /><button onClick={() => updateCall(call.id, { timeOut: nowTime() })}>Now</button></div></label><label className="call-time-in"><span>Time in</span><div className="time-now"><input type="time" step="60" value={call.timeIn} onChange={(event) => updateCall(call.id, { timeIn: event.target.value })} /><button onClick={() => updateCall(call.id, { timeIn: nowTime() })}>Now</button></div></label><label className="call-units"><span>Responding units</span><input placeholder="1203, 1205…" value={call.respondingUnits} onChange={(event) => updateCall(call.id, { respondingUnits: event.target.value })} /></label><label className="call-address"><span>Address</span><input list="known-addresses" autoComplete="street-address" placeholder="Start typing an address…" value={call.address} onChange={(event) => updateCall(call.id, { address: event.target.value })} /></label><label className="call-type"><span>Type</span><select value={call.callType} onChange={(event) => updateCall(call.id, { callType: event.target.value })}>{callTypes.map((type) => <option key={type}>{type}</option>)}</select></label>{calls.length > 1 && <button className="remove-call" aria-label={`Remove call ${index + 1}`} onClick={() => { setCalls((current) => current.filter((item) => item.id !== call.id)); markDirty(); }}>×</button>}</div>)}</div><button className="add-call bottom" onClick={() => { setCalls((current) => [...current, blankCall()]); markDirty(); }}>＋ Add Another Call</button></article>
