  ...
  const load = useCallback(async function loadSnapshot(sections?: readonly OperationalSection[]): Promise<void> {
    ...
    try {
      ...
      const fleetStatuses = (fleetResult.payload?.apparatus ?? []).map((unit) => ({
        ...
        status: committed.has(normalizeFleetApparatusName(unit.unitNumber)) ? "Committed to call" : unit.status.toLowerCase().replaceAll(/[\s_-]/g, "") === "inservice" ? "Available" : unit.status || "Status not reported",
      }));
