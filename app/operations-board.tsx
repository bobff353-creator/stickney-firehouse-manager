...
import BoardActiveCalls from './board-active-calls';
...
export default function OperationsBoard({ tvMode: stationTvMode = false, onTvModeChange, onNewActiveCall }: { tvMode?: boolean; onTvModeChange?: (enabled: boolean) => void; onNewActiveCall?: (call: BoardData["activeCalls"][number]) => void }) {
  ...
  useEffect(() => {
    if (!tvMode) return;
    const visibility = createTvExitVisibility(setTvExitVisible, (callback, delay) => window.setTimeout(callback, delay), (id) => window.clearTimeout(id));
  ...
  const load = useCallback(async function loadSnapshot(sections?: readonly OperationalSection[]): Promise<void> {
    if (loadInProgressRef.current) { reloadRequested.current = true; return; }
    loadInProgressRef.current = true;
    const controller = new AbortController();
    loadControllerRef.current = controller;
    const timeout = window.setTimeout(() => controller.abort(), 15000);
    try {
      const parts = sections?.filter(section => ['dashboard', 'duties', 'fleet'].includes(section));
      const query = parts?.length && confirmedBundle.current.dashboard?.ok ? `?parts=${parts.toSorted().join(',')}` : '';
      const response = await packetReader.current.read(`/api/live-operations${query}`, { signal: controller.signal });
      const received = await response.json() as BoardBundle;
      const bundle = response.ok ? { ...confirmedBundle.current, ...received } : received;
      ...
      confirmedBundle.current = bundle;
      ...
      const fleetStatuses = (fleetResult.payload?.apparatus ?? []).map((unit) => ({
        ...
        status: committed.has(normalizeFleetApparatusName(unit.unitNumber)) ? "Committed to call" : unit.status.toLowerCase().replaceAll(/[\s_-]/g, "") === "inservice" ? "Available" : operationalStatusLabel(unit.status),
      }));
      setData({ ...result, apparatus: fleetStatuses.length ? fleetStatuses : result.apparatus });
      setLastRefresh(new Date());
      setError([dashboard, dutiesResult, fleetResult].every(part => part.ok) ? '' : 'Some board information could not refresh. Retrying the last confirmed information.');
  ...
  const liveConnected = useOperationalUpdates({ scope: 'board', sections: ['dashboard', 'duties', 'fleet'], refresh: load, fallbackMs: 30_000, nextChangeAt, readFailed: Boolean(error) || !data });
  ...
  const feedDegraded=Boolean(error)||!lastRefresh||(!liveConnected&&clock.getTime()-lastRefresh.getTime()>75000);
  ...
  return <section className={`operations-board${tvMode ? " tv-display" : ""}${previewConfiguration ? ' board-configuration-preview' : ''}${announcementIsActive(activeConfiguration, clock.getTime()) ? ' has-board-announcement' : ''}`}>
    ...
    <div className="board-summary"><article className={data?.staffing.complete ? "clear" : "warning"}><span>Staffing</span><strong>{data?.staffing.filled ?? "—"} / {data?.staffing.required ?? "—"}</strong><small>{!data ? "Staffing not confirmed" : data.staffing.complete ? "Complete" : "Coverage needs attention"}</small></article><article className={data?.officerInCharge ? "clear" : "warning"}><span>Officer in charge</span><strong>{!data ? "Not confirmed" : data.officerInCharge ? displayName(data.officerInCharge) : "Not signed in"}</strong><small>Current shift command</small></article><BoardActiveCalls calls={data?.activeCalls ?? []} tvMode={tvMode} now={clock.getTime()} confirmed={Boolean(data)} /><article><span>Next shift change</span><strong>{next.label}</strong><small>In {next.remaining}</small></article></div>
    ...
    <div className="board-grid redesigned"><ChiefBoardPanel onBoardLinks={receiveBoardLinks} onBoardConfiguration={receiveConfiguration} tvMode={tvMode} manageRequest={manageNotesRequest}/>
      ...
      <section className={`board-panel equipment rotating-panel ${linkStyles.panel} ${rotation}${rotation === "duty" && dailyChecksNeedAttention ? " daily-check-alert" : ""}`} aria-live="polite">
        ...
        <div className="rotation-content">
          <div className="rotation-slide" hidden={rotation !== "equipment"}>{data?.equipmentIssues.length ? data.equipmentIssues.map((issue) => <article key={issue.id || issue.item}><b>{issue.item}</b><strong>{operationalStatusLabel(issue.status)}</strong><p>{issue.detail || "No details entered"}</p></article>) : tvMode || !data ? <BoardEquipmentSummary apparatus={data?.apparatus} confirmedAt={lastRefresh?.toISOString() ?? null} delayed={feedDegraded}/> : <p className="board-empty clear">✓ No equipment issues reported</p>}</div>
