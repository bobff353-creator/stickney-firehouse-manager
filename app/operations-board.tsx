...
type CurrentDuty = { dayOfWeek: number; shiftKey: "morning" | "afternoon" | "night"; duty: string; fleetChecks?: Array<{ apparatusId: string; unit: string; checkType: "daily" | "weekly" | "inventory" | "air_pack"; startTime: string; endTime: string; status: "pending" | "in_progress" | "completed" }> };
...
export default function OperationsBoard({ tvMode = false, onTvModeChange, onNewActiveCall }: { tvMode?: boolean; onTvModeChange?: (enabled: boolean) => void; onNewActiveCall?: (call: BoardData["activeCalls"][number]) => void }) {
  ...
  const weeklyChecksCompleted = scheduledWeeklyChecks.length > 0 && scheduledWeeklyChecks.every((check) => check.status === "completed");
