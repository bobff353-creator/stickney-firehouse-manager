...
import { hasConstructionProfile, hasOccupancyProfile, type ConstructionProfile, type OccupancyProfile } from "./preplans/profiles";
...
type RespondData = {
  ...
  operational:null|{levels:Array<{id:string;name:string;shortLabel:string;isDefault:number}>;spaces:Array<{id:string;levelId:string;name:string}>;alerts:Array<{id:string;title:string;message:string;severity:string;levelId?:string;spaceId?:string;expiresAt?:string}>;hazmat:Array<{id:string;materialName:string;unNumber:string;ergGuideNumber:string;quantity?:number;quantityUnit?:string;levelId?:string;spaceId?:string}>;hoseLays:Array<{id:string;name:string;recommendedHoseFeet:number;hoseSizeInches:number;supplyLineLabel:string}>;construction:ConstructionProfile;occupancy:OccupancyProfile;revision?:{revisionNumber:number;publishedAt:string};roomMatch?:{room:{id:string;name:string;levelId?:string}|null;confidence:number;reason:string}};
};
...
export default function Respond({ apparatus = "", onNavigate }: { apparatus?: string; onNavigate?:(page:"Daily Log"|"Field Preplans"|"Box Cards")=>void }) {
  ...
  return <section ref={pageRef} className={`respond-page${monitorMode?" monitor-view":""}`}>
    ...
    {data?.operational&&<section className="respond-operational-banner" aria-label="Published operational preplan intelligence"><header><span>PREPLAN 2.0 · REV {data.operational.revision?.revisionNumber??"LEGACY"}</span><strong>{data.operational.roomMatch?.room?`CAD room match: ${data.operational.roomMatch.room.name}`:"No reliable CAD room match"}</strong></header><div>{data.operational.alerts.slice(0,3).map((alert)=><article key={alert.id} className={alert.severity}><b>{alert.severity.toUpperCase()}</b><strong>{alert.title}</strong><span>{alert.message}</span></article>)}{data.operational.hazmat.slice(0,3).map((item)=><article key={item.id} className="hazmat"><b>HAZMAT</b><strong>{item.materialName}</strong><span>{item.unNumber?`UN/NA ${item.unNumber}`:"UN/NA not entered"} · {item.ergGuideNumber?`ERG ${item.ergGuideNumber}`:"Verify in official ERG"}</span></article>)}{hasConstructionProfile(data.operational.construction)&&<article className={data.operational.construction.bowstringTruss==="yes"||data.operational.construction.lightweightConstruction==="yes"?"critical":"construction"}><b>VERIFIED CONSTRUCTION</b><strong>{data.operational.construction.bowstringTruss==="yes"?"Bowstring truss":data.operational.construction.constructionType||data.operational.construction.roofType||"Construction profile"}</strong><span>{[data.operational.construction.roofSupportSystem,data.operational.construction.lightweightConstruction==="yes"?"Lightweight construction":data.operational.construction.basementType].filter(Boolean).join(" · ")}</span></article>}{hasOccupancyProfile(data.operational.occupancy)&&<article className="occupancy"><b>VERIFIED OCCUPANCY</b><strong>{data.operational.occupancy.classification||"Occupancy profile"}</strong><span>{[data.operational.occupancy.sleepingOccupants==="yes"?"Sleeping occupants":null,data.operational.occupancy.nonAmbulatory==="yes"?"Non-ambulatory occupants":null,data.operational.occupancy.assistanceNeeded==="yes"?"Assistance needed":null,data.operational.occupancy.peakOccupancy!=null?`Peak ${data.operational.occupancy.peakOccupancy}`:null].filter(Boolean).join(" · ")||"Verified profile available"}</span></article>}</div></section>}
