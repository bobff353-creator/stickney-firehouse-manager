...
export default function FieldPreplans() {
  ...
  async function save(payload:object,success:string){
    ...
    } catch(error){setMessage((error instanceof Error?error.message:"Unable to save")+". Your unsaved edits remain on this screen.");}
    finally{setBusy(false);}
  }
  ...
  return <section className={`field-preplans-page${recordFocused?" preplan-builder-focused":""}${hydrantDraft?" hydrant-record-focused":""}${writingDetails?' preplan-form-focused':''}`}>
    ...
    {draft&&recordMode==="edit"&&<section className="preplan-editor" ref={editorPanel}>
      ...
      <footer className="preplan-workflow-footer task-action-bar"><button type="button" disabled={busy} onClick={closePreplan}>← Back to Preplan list</button>{tab==='quick'&&quickStep===3?<><p className="task-action-help">{!footprintAccepted ? "Accept the building footprint before saving." : !draft.businessName.trim() ? "Enter a building name in Step 2 before saving." : "Save these building details, then add equipment locations."}</p><button type="button" className="primary-action" disabled={busy||savedReloadNeeded||!footprintAccepted||!draft.businessName.trim()} onClick={()=>void savePlan('details')}>{busy ? "Saving…" : "Save & continue to equipment →"}</button></>:tab==='details'?<button type="button" onClick={()=>switchSection('photos')}>Next: A–D photos & symbols →</button>:current&&<button type="button" disabled={busy||savedReloadNeeded} onClick={()=>view(current)}>Preview saved preplan</button>}</footer>
