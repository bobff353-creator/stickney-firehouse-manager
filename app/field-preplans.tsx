...
export default function FieldPreplans() {
  ...
  async function savePlan(continueTo?:"details"|"photos"){
    if(!draft||!footprintAccepted||busy)return;
    ...
    const id=await save({action:"savePreplan",id:draft.id,importId:selectedImport,businessName:draft.businessName,address,location:{lat:draft.latitude,lng:draft.longitude},aSide:draft.aSideLatitude==null?null:{lat:draft.aSideLatitude,lng:draft.aSideLongitude},footprint:draft.footprint,contactInfo,construction:draft.construction,accessInfo:draft.accessInfo,alarmSystem:draft.alarmSystem,knoxBox:draft.knoxBox,riser:draft.riser,fdc:draft.fdc,sprinklerSystem:draft.sprinklerSystem,floorCount:draft.floorCount,constructionType:draft.constructionType,occupancyFlowCategory:draft.occupancyFlowCategory,sprinklerStandard:draft.sprinklerStandard,status},"Building details saved. Next: map equipment, add exterior photos, or preview the saved preplan.");
    if(id){const next={...draft,address,contactInfo,id,status};setSelectedImport("");setSelected(id);setDraft(next);setSavedForm(formSignature(next));openPreplanUrl(id,true,true);if(continueTo)switchSection(continueTo);}
  }
  ...
  return <section className={`field-preplans-page${recordFocused?" preplan-builder-focused":""}${hydrantDraft?" hydrant-record-focused":""}${writingDetails?' preplan-form-focused':''}`}>
    {focusedPreplan&&draft&&<>
      <header className="preplan-focus-header"><button disabled={busy} onClick={closePreplan}>&larr; Back to Preplan list</button><div><span>{draft.id?(recordMode==="view"?"VIEW PREPLAN":"EDIT PREPLAN"):"NEW PREPLAN"}</span><h1>{draft.businessName||"New building preplan"}</h1><p>{fullAddress(draft)||"A-side GPS location"} &middot; {draft.status}</p></div>{recordMode==="edit"&&<div className="preplan-save-bar"><strong role="status">{busy?'Saving…':formDirty?'Unsaved building changes':draft.id?'Building details saved':'Not saved yet'}</strong><button className="primary-action" disabled={busy||!footprintAccepted||!formDirty} onClick={()=>void savePlan()}>{draft.id?'Save building changes':'Save new preplan'}</button>{current&&<button className="record-focus-view-button" disabled={busy} onClick={()=>view(current)}>Preview saved preplan</button>}</div>}</header>
    ...
    {draft&&recordMode==="edit"&&<section className="preplan-editor" ref={editorPanel}>
      ...
      <footer className="preplan-workflow-footer"><button type="button" disabled={busy} onClick={closePreplan}>← Back to Preplan list</button>{tab==='quick'&&quickStep===3?<button type="button" className="primary-action" disabled={busy||!footprintAccepted||!draft.businessName.trim()} onClick={()=>void savePlan('details')}>Save & continue to equipment →</button>:tab==='details'?<button type="button" onClick={()=>switchSection('photos')}>Next: A–D photos & symbols →</button>:current&&<button type="button" disabled={busy} onClick={()=>view(current)}>Preview saved preplan</button>}</footer>
