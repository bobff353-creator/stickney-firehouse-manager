...
function PreplanRecordView({plan,canEdit,onEdit,onShowFeature,mapDraft,onMapOverlayChange,onMapDrawingStart,onMapDrawingChange}:{plan:Preplan;canEdit:boolean;onEdit:()=>void;onShowFeature:(feature:Feature)=>void;mapDraft:OperationalMapDraft;onMapOverlayChange:(overlay:OperationalMapOverlay)=>void;onMapDrawingStart:(kind:"space"|"hoseLay",levelId:string)=>void;onMapDrawingChange:(draft:OperationalMapDraft)=>void}){
  ...
  return <section className="preplan-record-view" aria-label={`${plan.businessName} preplan record`}>
    ...
    <OperationalPreplanPanel preplanId={plan.id} canEdit={canEdit} mapDraft={mapDraft} onMapOverlayChange={onMapOverlayChange} onMapDrawingStart={onMapDrawingStart} onMapDrawingChange={onMapDrawingChange}/>
  </section>;
}
...
export default function FieldPreplans() {
  ...
  return <section className={`field-preplans-page${recordFocused?" preplan-builder-focused":""}`}>
    ...
    {current&&recordMode==="view"&&<PreplanRecordView plan={current} canEdit={canEdit} onEdit={()=>edit(current)} onShowFeature={(item)=>{setCenter({lat:item.latitude,lng:item.longitude});setZoom(20);focusMapPanel.current?.scrollIntoView({behavior:"smooth",block:"start"});}} mapDraft={operationalMapDraft} onMapOverlayChange={handleOperationalOverlay} onMapDrawingStart={startOperationalDrawing} onMapDrawingChange={changeOperationalDrawing}/>}
