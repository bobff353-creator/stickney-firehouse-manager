...
type ImportedBuilding = { id:string;businessName:string;address:string;sourceFile:string;sourceRow:number;sourceExternalId?:string;sourcePayload?:Record<string,unknown>;status:string;latitude:number|null;longitude:number|null;geocodeNote:string;linkedPreplanId:string|null };
...
function PreplanRecordView({plan,canEdit,onEdit,onShowFeature,onEditPhoto}:{plan:Preplan;canEdit:boolean;onEdit:()=>void;onShowFeature:(feature:Feature)=>void;onEditPhoto?:(photo:Photo)=>void}){
  const [readerSection,setReaderSection]=useState("overview");
  ...
  return <section className="preplan-record-view" aria-label={`${plan.businessName} preplan record`}>
    ...
    <nav className="preplan-reader-tabs no-print" aria-label="Building record sections">{[["overview","Building"],["systems",`Systems (${plan.features.length})`],["photos",`A–D photos (${exteriorPhotos.length})`],["contacts","Contacts"]].map(([key,label])=><button key={key} type="button" aria-pressed={readerSection===key} onClick={()=>setReaderSection(key)}>{label}</button>)}</nav>
    <div className="preplan-record-grid">
      <article hidden={readerSection!=="overview"} className="preplan-record-card preplan-record-building">
      ...
      <article hidden={readerSection!=="contacts"} className="preplan-record-card preplan-record-contacts">
      ...
      <article hidden={readerSection!=="overview"&&readerSection!=="systems"} className="preplan-record-card preplan-record-quick-systems">
    ...
    <article hidden={readerSection!=="systems"} className="preplan-record-card preplan-record-features">
    ...
    <article hidden={readerSection!=="photos"} className="preplan-record-card preplan-record-photos">
      <header><span>5</span><div><h3>Building exterior photos</h3><p>A, B, C and D sides</p></div><b>{exteriorPhotos.length}</b></header>
      {exteriorPhotos.length?<div>{exteriorPhotos.map((photo)=><figure key={photo.id}><IllustratedPhotoView photo={photo} alt={photo.caption||`${photo.side} side`}/><figcaption>{photo.side} Side{photo.caption?` · ${photo.caption}`:""}</figcaption>{onEditPhoto&&<button type="button" onClick={()=>onEditPhoto(photo)}>Edit photo symbols & caption</button>}</figure>)}</div>:<p className="preplan-record-empty">No exterior photos have been added.</p>}
    </article>
  </section>;
}

function FieldMap({ apiKey,center,zoom,imagery,plans,hydrants,selected,draft,mode,footprintAccepted,operationalOverlay,operationalDraft,selectedOperationalSpaceId,onMapClick,onSelect,onHydrantSelect,onCenter,onZoom,onProviderChange,onOperationalDraftChange,onOperationalSpaceSelect }:{ apiKey:string;center:Point;zoom:number;imagery:"aerial"|"street";plans:Preplan[];hydrants:Hydrant[];selected:string;draft:Form|null;mode:string;footprintAccepted:boolean;operationalOverlay:OperationalMapOverlay|null;operationalDraft:OperationalMapDraft;selectedOperationalSpaceId?:string;onMapClick:(point:Point)=>void;onSelect:(id:string)=>void;onHydrantSelect:(id:string)=>void;onCenter:(point:Point)=>void;onZoom:(zoom:number)=>void;onProviderChange:(provider:"google"|"fallback")=>void;onOperationalDraftChange?:(draft:OperationalMapDraft)=>void;onOperationalSpaceSelect?:(id:string)=>void }) {
  ...
  const operationalDrag=useRef<{kind:"space";start:Point;points:OperationalMapPoint[]}|{kind:"corner";index:number}|null>(null);
  ...
  const editingSpaceId=operationalDraft?.kind==="space"?operationalDraft.spaceId:"";
  const mappedSpaces=(operationalOverlay?.spaces??[]).filter((item)=>item.id!==editingSpaceId).map((item)=>({...item,points:operationalPoints(item.geometry,selectedFootprint)})).filter((item)=>item.points.length>=3);
  ...
  const renderHydrantPin=({hydrant,p}:(typeof mappedHydrants)[number])=><button type="button" key={hydrant.id} className={`hydrant-map-pin ${hydrant.serviceStatus}`} style={{left:`${p.x/width*100}%`,top:`${p.y/height*100}%`,scale:String(Math.max(.7,Math.min(1.15,.72+(zoom-17)*.14)))}} title={`${hydrant.hydrantNumber||"Hydrant"} · ${hydrant.serviceStatus.replaceAll("_"," ")}`} onPointerDown={(event)=>event.stopPropagation()} onClick={(event)=>{event.stopPropagation();onHydrantSelect(hydrant.id);}}><HydrantIcon outOfService={hydrant.serviceStatus==="out_of_service"}/></button>;
  ...
  function moveOperationalDraft(clientX:number,clientY:number){
    if(!operationalDraft||!operationalDrag.current||!onOperationalDraftChange||!mapRoot.current)return;
    const point=mapPoint(mapRoot.current,clientX,clientY),drag=operationalDrag.current;
    if(drag.kind==="corner"){const points=[...operationalPoints(operationalDraft.points,selectedFootprint)];points[drag.index]=point;onOperationalDraftChange({...operationalDraft,points});return;}
    const deltaLat=point.lat-drag.start.lat,deltaLng=point.lng-drag.start.lng;
    const points=drag.points.map((savedPoint)=>"lat" in savedPoint?{lat:savedPoint.lat+deltaLat,lng:savedPoint.lng+deltaLng}:savedPoint);
    onOperationalDraftChange({...operationalDraft,points});
  }
  return <div ref={mapRoot} className={`field-map ${mode ? "capture" : ""}${dragging ? " dragging" : ""}${googleReady ? " google-active" : ""}${operationalDraft ? " operational-drawing" : ""}`}
    ...
    <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" aria-label="Preplan map overlay">
      ...
      {mappedSpaces.map((space)=><g className={`operational-space-map-overlay${selectedOperationalSpaceId===space.id?" selected":""}`} key={`space-${space.id}`}><polygon points={polygon(space.points,center,zoom,width,height)} role={onOperationalSpaceSelect?"button":undefined} tabIndex={onOperationalSpaceSelect?0:undefined} aria-label={onOperationalSpaceSelect?`Select room ${String(space.name??"Room")}`:undefined} onClick={(event)=>{if(!onOperationalSpaceSelect)return;event.stopPropagation();onOperationalSpaceSelect(space.id);}} onKeyDown={(event)=>{if(onOperationalSpaceSelect&&(event.key==="Enter"||event.key===" ")){event.preventDefault();onOperationalSpaceSelect(space.id);}}}/><text x={project(space.points.reduce((sum,point)=>({lat:sum.lat+point.lat/space.points.length,lng:sum.lng+point.lng/space.points.length}),{lat:0,lng:0}),center,zoom,width,height).x} y={project(space.points.reduce((sum,point)=>({lat:sum.lat+point.lat/space.points.length,lng:sum.lng+point.lng/space.points.length}),{lat:0,lng:0}),center,zoom,width,height).y}>{String(space.name??"Room")}</text></g>)}
      ...
      {operationalDraft?.kind==="space"&&mappedOperationalDraft.length>0&&<><polygon className="operational-space-map-overlay draft" points={polygon(mappedOperationalDraft,center,zoom,width,height)} onPointerDown={(event)=>{if(mappedOperationalDraft.length<3)return;event.stopPropagation();event.currentTarget.setPointerCapture(event.pointerId);if(mapRoot.current)operationalDrag.current={kind:"space",start:mapPoint(mapRoot.current,event.clientX,event.clientY),points:mappedOperationalDraft};}} onPointerMove={(event)=>moveOperationalDraft(event.clientX,event.clientY)} onPointerUp={()=>{operationalDrag.current=null;}} onPointerCancel={()=>{operationalDrag.current=null;}}/>{mappedOperationalDraft.map((point,index)=>{const p=project(point,center,zoom,width,height);return <circle className="operational-map-corner" key={`space-draft-${index}`} cx={p.x} cy={p.y} r="8" onPointerDown={(event)=>{event.stopPropagation();event.currentTarget.setPointerCapture(event.pointerId);operationalDrag.current={kind:"corner",index};}} onPointerMove={(event)=>moveOperationalDraft(event.clientX,event.clientY)} onPointerUp={()=>{operationalDrag.current=null;}} onPointerCancel={()=>{operationalDrag.current=null;}}/>;})}</>}
    ...
    {showIndividual?mappedHydrants.map(renderHydrantPin):clusters.map((group)=>{if(group.length===1)return renderHydrantPin(group[0]);const x=group.reduce((sum,item)=>sum+item.p.x,0)/group.length,y=group.reduce((sum,item)=>sum+item.p.y,0)/group.length;return <button type="button" key={group.map((item)=>item.hydrant.id).join("-")} className="hydrant-cluster" style={{left:`${x/width*100}%`,top:`${y/height*100}%`}} aria-label={`${group.length} hydrants · Zoom in`} onPointerDown={(event)=>event.stopPropagation()} onClick={(event)=>{event.stopPropagation();onCenter({lat:group[0].hydrant.latitude,lng:group[0].hydrant.longitude});onZoom(Math.min(20,zoom+2));}}><i/><b>×{group.length}</b></button>;})}
...
export default function FieldPreplans() {
  ...
  const [savedForm,setSavedForm]=useState(''),[operationalDirty,setOperationalDirty]=useState(false);
  const [savedReloadNeeded,setSavedReloadNeeded]=useState(false);
  const [buildingSaveError,setBuildingSaveError]=useState("");
  const [canManageAttachments,setCanManageAttachments]=useState(false),[editingPhoto,setEditingPhoto]=useState<Photo|null>(null);
  const editorPanel=useRef<HTMLElement|null>(null);
  const [directoryView,setDirectoryView]=useWorkspaceViewState<"map"|"starters">("preplan-directory","map"),[quickStep,setQuickStep]=useState<1|2|3>(1),[detailsStep,setDetailsStep]=useState<"overview"|"add"|"systems">("overview"),[photoSide,setPhotoSide]=useState<"A"|"B"|"C"|"D">("A");
  ...
  const [editingFeatureId,setEditingFeatureId]=useState(''),[featurePoint,setFeaturePoint]=useState<Point|null>(null);
  ...
  const load=useCallback(async()=>{const response=await fetch("/api/field-preplans",{cache:"no-store"});const body=await response.json() as {preplans?:Preplan[];imports?:ImportedBuilding[];canEdit?:boolean;canManageAttachments?:boolean;canDelete?:boolean;error?:string};if(!response.ok)throw new Error(body.error||"Unable to load preplans");setPlans(body.preplans??[]);setImports(body.imports??[]);setCanEdit(Boolean(body.canEdit));setCanManageAttachments(Boolean(body.canManageAttachments));setCanDeletePreplan(Boolean(body.canDelete));},[]);
  ...
  useEffect(()=>{
    const syncFocusedPreplan=(fromHistory=false)=>{
      ...
      if(selected!==id){const view=detailedPreplanMapView(plan.footprint,{lat:plan.latitude,lng:plan.longitude}),form=formFromPlan(plan);setSelectedImport("");setSelected(plan.id);setDraft(form);setSavedForm(formSignature(form));setFootprintAccepted(true);setCenter(view.center);setZoom(view.zoom);setTab("quick");setQuickStep(2);setHydrantDraft(null);setMode("");}
    };
    syncFocusedPreplan();
  ...
  const [recordScope,setRecordScope]=useWorkspaceViewState<"map"|"all">("preplan-record-scope","map");
  ...
  const shown=useMemo(()=>{const span=360/(2**zoom)*1.6;return plans.filter((plan)=>{
    ...
    return matchesSearch&&(normalizedQuery||recordScope==="all"?true:isOnMap);
  });},[plans,normalizedQuery,recordScope,center,zoom]);
  const shownHydrants=useMemo(()=>{const span=360/(2**zoom)*1.6;return hydrants.filter((item)=>{
    ...
    return matchesSearch&&(normalizedQuery||recordScope==="all"?true:isOnMap);
  });},[hydrants,normalizedQuery,recordScope,center,zoom]);
  ...
  const formDirty=Boolean(draft&&recordMode==='edit'&&formSignature(draft)!==savedForm);
  const leavingDirty=formDirty||operationalDirty||Boolean(featurePhoto)||Boolean(tab==='details'&&detailsStep==='add'&&(editingFeatureId||feature.label||feature.details))||Boolean(mode&&mode!=='footprint'&&mode!=='aSide');
  useEffect(()=>{if(!leavingDirty)return;const warn=(event:BeforeUnloadEvent)=>{event.preventDefault();};window.addEventListener('beforeunload',warn);return()=>window.removeEventListener('beforeunload',warn);},[leavingDirty]);
  function canLeave(){return !busy&&(!leavingDirty||window.confirm('Leave this preplan and discard unsaved edits? Previously saved information will stay unchanged.'));}
  function resetFeatureEditor(){setEditingFeatureId('');setFeaturePoint(null);setFeaturePhoto(null);setFeature({featureType:'knox',label:'',systemType:'',serviceStatus:'in_service',details:''});}
  function switchSection(next:typeof tab){if(busy)return;if(operationalDirty&&!window.confirm('Close the open advanced editor? Unsaved edits in that editor will be discarded.'))return;setOperationalDirty(false);setTab(next);setMode('');setOperationalMapDraft(null);requestAnimationFrame(()=>editorPanel.current?.scrollIntoView({block:'start',behavior:'smooth'}));}
  function nextQuickStep(next:1|2|3){setQuickStep(next);setMode('');requestAnimationFrame(()=>editorPanel.current?.scrollIntoView({block:'start',behavior:'smooth'}));}
  function photoSaved(photo:Photo){setPlans(items=>items.map(plan=>plan.id===selected?{...plan,photos:plan.photos.map(item=>item.id===photo.id?photo:item)}:plan));setMessage('Photo symbols and caption saved. The original image is unchanged.');}
  async function openPhotoEditor(photo:Photo){
    if(busy)return;setBusy(true);setMessage('Checking the latest saved photo details…');
    try{const response=await fetch(`/api/field-preplans/photos/${encodeURIComponent(photo.id)}/illustrations`,{cache:'no-store',signal:AbortSignal.timeout(12000)});const body=await response.json() as Partial<Photo>&{error?:string};if(!response.ok)throw new Error(body.error||'Could not check the latest photo changes.');const latest={...photo,...body};setPlans(items=>items.map(plan=>plan.id===selected?{...plan,photos:plan.photos.map(item=>item.id===photo.id?latest:item)}:plan));setEditingPhoto(latest);setMessage('');}catch(error){setMessage(error instanceof Error?error.message:'Could not check the latest photo changes. Try again.');}finally{setBusy(false);}
  }
  ...
  async function save(payload:object,success:string){
    setBusy(true);setMessage("");setBuildingSaveError("");
    try {
      const response=await fetch("/api/field-preplans",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});
      const body=await response.json() as {id?:string;error?:string};
      if(!response.ok)throw new Error(body.error||"Unable to save");
      try {await load();setSavedReloadNeeded(false);setMessage(success);return {id:body.id,refreshed:true};}
      catch {setSavedReloadNeeded(true);setMessage("Building details were saved, but the saved record could not be reloaded. Reload before previewing or continuing. Your record ID is retained; do not create another preplan.");return {id:body.id,refreshed:false};}
    } catch(error){const detail=(error instanceof Error?error.message:"Unable to save")+". Your unsaved edits remain on this screen.";setMessage(detail);setBuildingSaveError(detail);}
    finally{setBusy(false);}
  }
  async function savePlan(continueTo?:"details"|"photos"){
    if(!draft||!footprintAccepted||busy||savedReloadNeeded)return;
    if(draft.id&&!formDirty){if(continueTo)switchSection(continueTo);return;}
    if(!draft.businessName.trim()){setMessage('Enter the business / building name before saving.');nextQuickStep(2);return;}
    const address=fullAddress(draft),contactInfo=serializePreplanContacts(draft.contacts),status=selectedImport?'Quick Preplan':draft.status;
    const result=await save({action:"savePreplan",id:draft.id,importId:selectedImport,businessName:draft.businessName,address,location:{lat:draft.latitude,lng:draft.longitude},aSide:draft.aSideLatitude==null?null:{lat:draft.aSideLatitude,lng:draft.aSideLongitude},footprint:draft.footprint,contactInfo,construction:draft.construction,accessInfo:draft.accessInfo,alarmSystem:draft.alarmSystem,knoxBox:draft.knoxBox,riser:draft.riser,fdc:draft.fdc,sprinklerSystem:draft.sprinklerSystem,floorCount:draft.floorCount,constructionType:draft.constructionType,occupancyFlowCategory:draft.occupancyFlowCategory,sprinklerStandard:draft.sprinklerStandard,status},"Building details saved. Next: map equipment, add exterior photos, or preview the saved preplan.");
    const id=result?.id;
    if(id){const next={...draft,address,contactInfo,id,status};setSelectedImport("");setSelected(id);setDraft(next);setSavedForm(formSignature(next));openPreplanUrl(id,true,true);if(continueTo&&result?.refreshed)switchSection(continueTo);}
  }
  async function saveFeature(point:Point,type=feature.featureType){
    ...
    try{
      const response=await fetch("/api/field-preplans",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"saveFeature",id:editingFeatureId||undefined,preplanId:selected,featureType:type,label:feature.label,systemType:feature.systemType,serviceStatus:feature.serviceStatus,details:feature.details,location:point})});
      const body=await response.json() as {id?:string;error?:string};
      ...
      setEditingFeatureId('');setFeaturePoint(null);
  ...
  return <section className={`field-preplans-page${recordFocused?" preplan-builder-focused":""}${hydrantDraft?" hydrant-record-focused":""}${writingDetails?' preplan-form-focused':''}`}>
    {focusedPreplan&&draft&&<>
      ...
      <div className="preplan-focus-map-panel" ref={focusMapPanel}>
        <div className="field-map-toolbar"><button onClick={locate}>◎ Current location</button><button className={imagery==="aerial"?"active":""} onClick={()=>setImagery("aerial")}>Aerial</button><button className={imagery==="street"?"active":""} onClick={()=>setImagery("street")}>Streets</button><small>Drag to move &middot; wheel or double-click to zoom</small><em className={`map-provider ${mapProvider}`}>{mapProvider==="google"?`Google Maps · ${imagery==="aerial"?"Satellite":"Streets"}`:mapProvider==="loading"?"Loading map…":"Backup map"}</em><div className="preplan-map-zoom" role="group" aria-label="Map zoom"><button aria-label="Zoom out" onClick={()=>setZoom(Math.max(14,zoom-1))}>−</button><b>Zoom {zoom}</b><button aria-label="Zoom in" onClick={()=>setZoom(Math.min(21,zoom+1))}>+</button></div></div>
        {capturingFootprint&&<p className="preplan-capture-help"><strong>Draw the building outline</strong>Click or tap each outside corner in order. Hydrants and other saved map markers are temporarily hidden; they return after you accept the footprint.</p>}
        <FieldMap apiKey={mapsApiKey} center={center} zoom={zoom} imagery={imagery} plans={capturingFootprint?[]:mapPlans} hydrants={capturingFootprint?[]:hydrants} selected={selected} draft={draft} mode={operationalMapDraft?`operational-${operationalMapDraft.kind}`:recordMode==="edit"?mode:""} footprintAccepted={footprintAccepted} operationalOverlay={operationalOverlay} operationalDraft={operationalMapDraft} selectedOperationalSpaceId={selectedOperationalSpaceId} onMapClick={operationalMapDraft||recordMode==="edit"?clickMap:()=>{}} onCenter={setCenter} onZoom={setZoom} onProviderChange={setMapProvider} onHydrantSelect={()=>{}} onOperationalDraftChange={setOperationalMapDraft} onOperationalSpaceSelect={setSelectedOperationalSpaceId} onSelect={(id)=>{const plan=plans.find((item)=>item.id===id);if(plan&&plan.id!==selected)view(plan);}}/>
      </div>
    </>}
    {!recordFocused&&<>
...
<div className={`field-map-workspace${mapExpanded?" expanded":""}`}>
    ...
    {hydrantDraft&&<section className="hydrant-editor">
      <header>
        <div><span>WATER SUPPLY</span><h2>{hydrantDraft.hydrantNumber?`Hydrant ${hydrantDraft.hydrantNumber}`:"Quick Add Hydrant"}</h2><p>{hydrantDraft.address||`${hydrantDraft.latitude.toFixed(6)}, ${hydrantDraft.longitude.toFixed(6)}`}</p></div>
        <nav aria-label="Hydrant record sections">
          <button type="button" aria-pressed={hydrantTab==="quick"} className={hydrantTab==="quick"?"active":""} onClick={()=>setHydrantTab("quick")}>Quick Add</button>
          <button type="button" aria-pressed={hydrantTab==="details"} disabled={!hydrantDraft.id} className={hydrantTab==="details"?"active":""} onClick={()=>setHydrantTab("details")}>Hydrant Details</button>
          <button type="button" aria-pressed={hydrantTab==="flush"} disabled={!hydrantDraft.id} className={hydrantTab==="flush"?"active":""} onClick={()=>setHydrantTab("flush")}>Flushing</button>
          <button type="button" aria-pressed={hydrantTab==="flow"} disabled={!hydrantDraft.id} className={hydrantTab==="flow"?"active":""} onClick={()=>setHydrantTab("flow")}>NFPA 291 Flow Test</button>
        </nav>
      </header>
      ...
      {hydrantTab==="details"&&<div className="hydrant-details">
        <article className="content-card hydrant-profile">
          <header><h3>Hydrant profile</h3><p>Update the details below, then save your changes.</p></header>
          <label>Hydrant ID<input value={hydrantDraft.hydrantNumber} onChange={(event)=>setHydrantDraft({...hydrantDraft,hydrantNumber:event.target.value})}/></label>
          <label>Service status<select value={hydrantDraft.serviceStatus} onChange={(event)=>setHydrantDraft({...hydrantDraft,serviceStatus:event.target.value})}><option value="in_service">In service</option><option value="out_of_service">Out of service</option></select></label>
          <label className="hydrant-profile-wide">Nearest address<input aria-describedby="hydrant-address-help" value={hydrantDraft.address} onChange={(event)=>setHydrantDraft({...hydrantDraft,address:event.target.value})}/><small id="hydrant-address-help">For reference only. The map uses this hydrant’s saved GPS location.</small></label>
          <label>Manufacturer<input value={hydrantDraft.manufacturer} onChange={(event)=>setHydrantDraft({...hydrantDraft,manufacturer:event.target.value})}/></label>
          <label>Model<input value={hydrantDraft.model} onChange={(event)=>setHydrantDraft({...hydrantDraft,model:event.target.value})}/></label>
          <label className="hydrant-profile-wide">Port layout<select value={hydrantDraft.portCount} onChange={(event)=>{const count=Number(event.target.value);setHydrantDraft({...hydrantDraft,portCount:count,portSizes:Array.from({length:count},(_,index)=>hydrantDraft.portSizes[index]||"2.5")});}}><option value="0">Unknown — verify in field</option><option value="1">1 port</option><option value="2">2 ports</option><option value="3">3 ports</option></select></label>
          {hydrantDraft.portCount>0?<div className="hydrant-ports">{Array.from({length:hydrantDraft.portCount},(_,index)=><label key={index}>Port {index+1} size (in.)<input inputMode="decimal" value={hydrantDraft.portSizes[index]||""} onChange={(event)=>{const sizes=[...hydrantDraft.portSizes];sizes[index]=event.target.value;setHydrantDraft({...hydrantDraft,portSizes:sizes});}}/></label>)}</div>:<p className="field-message">Port layout is unverified. Check the hydrant in the field before relying on this detail.</p>}
          <label className="hydrant-profile-wide">Operational notes<textarea rows={5} value={hydrantDraft.notes} onChange={(event)=>setHydrantDraft({...hydrantDraft,notes:event.target.value})}/></label>
          <footer className="hydrant-profile-actions"><button type="button" className="primary-action" disabled={busy} onClick={()=>void saveHydrant()}>{busy?"Saving…":"Save Hydrant Details"}</button></footer>
        </article>
        <article className="content-card hydrant-summary" aria-label="Hydrant summary">
          <HydrantIcon outOfService={hydrantDraft.serviceStatus==="out_of_service"}/>
          <h3>Record summary</h3><strong className={hydrantDraft.serviceStatus}>{hydrantDraft.serviceStatus.replaceAll("_"," ")}</strong>
          <dl><div><dt>Make</dt><dd>{hydrantDraft.manufacturer||"Not entered"}</dd></div><div><dt>Model</dt><dd>{hydrantDraft.model||"Not entered"}</dd></div><div><dt>Ports</dt><dd>{hydrantDraft.portSizes.join('" · ')||"Unknown — verify"}{hydrantDraft.portSizes.length?'"':""}</dd></div><div><dt>Last flushed</dt><dd>{selectedHydrant?.flushes[0]?new Date(selectedHydrant.flushes[0].flushedAt).toLocaleDateString():"No record"}</dd></div></dl>
        </article>
      </div>}
    ...
    {draft&&recordMode==="edit"&&<section className="preplan-editor" ref={editorPanel}>
      ...
      {tab==="details"&&current&&<><nav className="preplan-subtabs" aria-label="Building system tasks"><button className={detailsStep==="overview"?"active":""} onClick={()=>setDetailsStep("overview")}><b>1</b> Building overview</button><button className={detailsStep==="add"?"active":""} onClick={()=>setDetailsStep("add")}><b>2</b> Add a feature</button><button className={detailsStep==="systems"?"active":""} onClick={()=>setDetailsStep("systems")}><b>3</b> Mapped systems <span>{current.features.length}</span></button></nav><div className="preplan-detail-grid preplan-single-detail">
        ...
        {detailsStep==="add"&&<article className="content-card">
          <h3>{editingFeatureId?'Edit mapped equipment':'Place an operational feature'}</h3>
          {editingFeatureId&&<p>Update these details and save. The equipment stays at its saved location unless you explicitly place it elsewhere.</p>}
          ...
          <div className="feature-placement-actions">
            {editingFeatureId&&featurePoint&&<button type="button" className="primary-action" disabled={busy} onClick={()=>void saveFeature(featurePoint)}>Save equipment changes</button>}
          ...
          <button type="button" disabled={busy} onClick={()=>{if((editingFeatureId||feature.label||feature.details||featurePhoto)&&!window.confirm('Discard unsaved equipment edits and return to saved equipment?'))return;resetFeatureEditor();setMode('');setDetailsStep('systems');}}>← Back to saved equipment</button>
        </article>}
        {detailsStep==="systems"&&<article className="content-card mapped-feature-list">
          {current.features.length>0&&<label>Edit a saved equipment record<select aria-label="Edit saved equipment" value="" onChange={event=>{const item=current.features.find(item=>item.id===event.target.value);if(!item)return;setEditingFeatureId(item.id);setFeaturePoint({lat:item.latitude,lng:item.longitude});setFeature({featureType:item.featureType,label:item.label,systemType:item.systemType,serviceStatus:item.serviceStatus,details:item.details});setDetailsStep('add');}}><option value="">Choose equipment to edit…</option>{current.features.map(item=><option key={item.id} value={item.id}>{item.label||pinMeta[item.featureType]?.label||'Equipment'}</option>)}</select></label>}
          ...
          <button type="button" className="mapped-feature-add-button" onClick={()=>{setEditingFeatureId('');setFeaturePoint(null);setFeature({featureType:'knox',label:'',systemType:'',serviceStatus:'in_service',details:''});setDetailsStep("add");}}>+ Add another operational feature</button>
        </article>}
      </div></>}
      {tab==="photos"&&current&&<>
        <nav className="preplan-photo-tabs" aria-label="Building side photos">{([['A','Alpha'],['B','Bravo'],['C','Charlie'],['D','Delta']] as const).map(([side,name])=><button key={side} className={photoSide===side?'active':''} aria-pressed={photoSide===side} onClick={()=>setPhotoSide(side)}><b>{side} · {name}</b><span>{current.photos.filter(photo=>photo.side===side&&!photo.featureId).length} photos</span></button>)}</nav>
        <div className="preplan-photo-grid preplan-single-photo"><article><header><div><b>{photoSide} Side photos</b><span>Show the full exterior side and important access points. Add optional symbols afterward.</span></div>{canManageAttachments?<div className="preplan-photo-upload-actions"><label>{busy?'Uploading…':'Take a photo'}<input type="file" accept="image/*" capture="environment" disabled={busy} onChange={event=>void upload(event,photoSide)}/></label><label>Choose existing photo<input type="file" accept="image/*" disabled={busy} onChange={event=>void upload(event,photoSide)}/></label></div>:<p>Photo editing permission is required to upload or annotate.</p>}</header>
          {current.photos.filter(photo=>photo.side===photoSide&&!photo.featureId).length?<div className="preplan-exterior-gallery">{current.photos.filter(photo=>photo.side===photoSide&&!photo.featureId).map(photo=><figure key={photo.id}><IllustratedPhotoView photo={photo} alt={`${photoSide} side ${photo.caption||photo.filename||''}`}/><figcaption>{photo.caption||'No caption added'}</figcaption>{canManageAttachments&&<button type="button" onClick={()=>void openPhotoEditor(photo)}>Edit photo symbols & caption</button>}</figure>)}</div>:<p>No {photoSide}-side photos yet. Take a photo or choose one from this device.</p>}
        </article></div>
        <div className="preplan-side-actions"><button disabled={photoSide==='A'} onClick={()=>setPhotoSide((['A','B','C','D'] as const)[['A','B','C','D'].indexOf(photoSide)-1])}>← Previous side</button>{photoSide!=='D'?<button onClick={()=>setPhotoSide((['A','B','C','D'] as const)[['A','B','C','D'].indexOf(photoSide)+1])}>Next: {(['A','B','C','D'] as const)[['A','B','C','D'].indexOf(photoSide)+1]}-side photos →</button>:<button onClick={()=>switchSection('operational')}>Optional: Advanced details & publication →</button>}</div>
      </>}
      {tab==="operational"&&current&&<div className="preplan-operational-edit"><OperationalPreplanPanel preplanId={current.id} canEdit={canEdit} onDirtyChange={setOperationalDirty} mapDraft={operationalMapDraft} selectedSpaceId={selectedOperationalSpaceId} onMapOverlayChange={handleOperationalOverlay} onMapDrawingStart={startOperationalDrawing} onMapDrawingChange={changeOperationalDrawing} onMapFocus={()=>focusMapPanel.current?.scrollIntoView({behavior:"smooth",block:"start"})} onSpaceSelect={setSelectedOperationalSpaceId}/></div>}
      <footer className="preplan-workflow-footer task-action-bar"><span>Preplans → {draft.businessName || "New building"} → {tab === "quick" ? "Building details" : tab === "details" ? "Map equipment" : tab === "photos" ? "Exterior photos" : "Advanced details"}</span>{tab==='quick'&&quickStep===3?<><p className="task-action-help">{!footprintAccepted ? "Accept the building footprint before saving." : !draft.businessName.trim() ? "Enter a building name in Step 2 before saving." : "Save these building details, then add equipment locations."}</p><button type="button" className="primary-action" disabled={busy||savedReloadNeeded||!footprintAccepted||!draft.businessName.trim()} onClick={()=>void savePlan('details')}>{busy ? "Saving…" : "Save & continue to equipment →"}</button></>:tab==='details'?<button type="button" onClick={()=>switchSection('photos')}>Next: A–D photos & symbols →</button>:current&&<button type="button" disabled={busy||savedReloadNeeded} onClick={()=>view(current)}>Preview saved preplan</button>}</footer>
    ...
    {editingPhoto&&<PhotoIllustrationEditor key={editingPhoto.id} photo={editingPhoto} onClose={()=>setEditingPhoto(null)} onSaved={photoSaved}/>}
  </section>;
}
