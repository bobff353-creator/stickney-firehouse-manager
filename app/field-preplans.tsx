...
export default function FieldPreplans() {
  ...
  function focusPlan(plan:Preplan,nextMode:"view"|"edit") {const view=detailedPreplanMapView(plan.footprint,{lat:plan.latitude,lng:plan.longitude});setSelectedImport("");setSelected(plan.id);setDraft(formFromPlan(plan));setFootprintAccepted(true);setCenter(view.center);setZoom(view.zoom);setTab("quick");setQuickStep(1);setDetailsStep("overview");setHydrantDraft(null);setMode("");setRecordMode(nextMode);setFocusedPreplan(true);openPreplanUrl(plan.id,false,nextMode==="edit");window.scrollTo({top:0,behavior:"smooth"});}
  ...
  function openHydrant(id:string){const item=hydrants.find((hydrant)=>hydrant.id===id);if(!item)return;setHydrantDraft({...item});setHydrantTab("details");setDraft(null);setSelected("");setCenter({lat:item.latitude,lng:item.longitude});setZoom(20);openHydrantUrl(id);window.scrollTo({top:0,behavior:"smooth"});}
  ...
  return <section className={`field-preplans-page${recordFocused?" preplan-builder-focused":""}`}>
    {focusedPreplan&&draft&&<>
      ...
      <div className="preplan-focus-map-panel" ref={focusMapPanel}>
        ...
        <FieldMap apiKey={mapsApiKey} center={center} zoom={zoom} imagery={imagery} plans={mapPlans} hydrants={[]} selected={selected} draft={draft} mode={recordMode==="edit"?mode:""} footprintAccepted={footprintAccepted} onMapClick={recordMode==="edit"?clickMap:()=>{}} onCenter={setCenter} onZoom={setZoom} onProviderChange={setMapProvider} onHydrantSelect={()=>{}} onSelect={(id)=>{const plan=plans.find((item)=>item.id===id);if(plan&&plan.id!==selected)view(plan);}}/>
      </div>
    </>}
    {!recordFocused&&<>
