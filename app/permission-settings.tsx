...
type PermissionData = {
  ...
  catalog: Array<{ key: string; label: string; group: string; required?: boolean; individualOnly?: boolean }>;
  ranks: string[];
  ...
  return <section className="permission-page">
    ...
    {tab === "permissions" ? <>
      <div className="content-card admin-editor-controls">
        ...
        <p className="permission-admin-note">Live Operations runs continuous board updates. Members need an individual grant; administrators retain access unless removed. Respond, call notifications and Road Closures are separate.</p>
        <button type="button" className="quiet-button" onClick={() => changeContext(() => { setEditor("member"); setSearch("Live Operations"); })}>Manage Live Operations access</button>
      ...
      {editor === "rank" && <article className="content-card permission-section admin-permission-editor"><header><div><h2>Permissions by rank</h2><p>Set starting access by rank. Administrators start with full access; use employee exceptions to restrict them.</p></div><select aria-label="Rank to configure" value={rank} disabled={saving || needsRefresh} onChange={(event) => changeContext(() => setRank(event.target.value))}>{data.ranks.map((item) => <option key={item}>{item}</option>)}</select></header>
        <div className="permission-groups">{groups.map((group) => <fieldset key={group}><legend>{group}</legend>{filtered.filter((permission) => permission.group === group).map((permission) => <label key={permission.key}><input type="checkbox" checked={permission.required || rankPermissions.has(permission.key)} disabled={permission.required || permission.individualOnly || saving || needsRefresh} onChange={(event) => { const next = new Set(rankPermissions); if (event.target.checked) next.add(permission.key); else next.delete(permission.key); setData({ ...data, rankSettings: { ...data.rankSettings, [rank]: [...next] } }); }}/><span>{permission.label}{permission.individualOnly && <small>Grant individually in Member exceptions, not by rank</small>}{permission.required && <small>Required for every employee</small>}</span></label>)}</fieldset>)}</div>
      ...
      {editor === "member" && <article className="content-card permission-section admin-permission-editor"><header><div><h2>Employee exceptions</h2><p>Add or subtract access for one employee without changing their rank.</p></div><select aria-label="Employee exceptions" value={employeeId} disabled={saving || needsRefresh} onChange={(event) => changeContext(() => setEmployeeId(event.target.value))}>{data.employees.map((employee) => <option value={employee.id} key={employee.id}>{employee.name} · {employee.rank}</option>)}</select></header>
        ...
        {Boolean(selectedEmployee?.isAdmin) && <p className="permission-admin-note">Administrators begin with full access. To limit a specific administrator, remove only the privileges they should not use. “Manage payroll, rates, and all timesheets” controls every employee timesheet and editing.</p>}
        <div className="permission-override-list">{filtered.map((permission) => { const effect = permission.required ? "allow" : data.overrides[employeeId]?.[permission.key] ?? "rank"; const baseAllowed = Boolean(selectedEmployee?.isAdmin) || (!permission.individualOnly && employeeRankPermissions.has(permission.key)); return <label key={permission.key}><span><b>{permission.label}</b><small>{permission.required ? "Required for every employee" : `${selectedEmployee?.isAdmin ? "Administrator default" : permission.individualOnly ? "Default (grant individually)" : "Rank"}: ${baseAllowed ? "Allowed" : "Not allowed"}`}</small></span><select value={effect} disabled={permission.required || !selectedEmployee || selectedEmployee.isOwner || saving || needsRefresh} onChange={(event) => { const next = { ...(data.overrides[employeeId] ?? {}) }; if (event.target.value === "rank") delete next[permission.key]; else next[permission.key] = event.target.value as "allow" | "deny"; setData({ ...data, overrides: { ...data.overrides, [employeeId]: next } }); }}><option value="rank">{selectedEmployee?.isAdmin ? "Use administrator default" : permission.individualOnly ? "Not allowed (default)" : "Use rank setting"}</option><option value="allow">Add permission</option><option value="deny">Remove permission</option></select></label>; })}</div>
