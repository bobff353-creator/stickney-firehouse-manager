...
export default function PermissionSettings({ initialTab, testEmployeeId, onTestEmployee, onPermissionsSaved }: { initialTab: "permissions" | "test"; testEmployeeId: string; onTestEmployee: (employee: PermissionData["employees"][number] | null) => void; onPermissionsSaved: (data: PermissionData) => void }) {
  ...
  const load = useCallback(async () => { const response = await fetch(`/api/permissions?refresh=${Date.now()}`, { cache: "no-store" }); const payload = await response.json() as PermissionData & { error?: string }; if (!response.ok) throw new Error(payload.error || "Unable to load permissions"); setData(payload); setRank((current) => current || payload.ranks[0] || ""); setEmployeeId((current) => current || payload.employees[0]?.id || ""); return payload; }, []);
  ...
  async function save(body: object) { setSaving(true); setMessage(""); try { const response = await fetch("/api/permissions", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(body) }); const payload = await response.json() as { error?: string; saved?: boolean }; if (!response.ok || !payload.saved) throw new Error(payload.error || "The permission change was not confirmed."); const refreshed = await load(); onPermissionsSaved(refreshed); setMessage(`Permissions saved and active · ${new Date().toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}`); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save permissions"); } finally { setSaving(false); } }
  ...
  return <section className="permission-page">
    ...
    {tab === "permissions" ? <>
      <article className="content-card permission-section"><header><div><h2>Permissions by rank</h2><p>Set the starting access for everyone assigned to a rank.</p></div><select value={rank} onChange={(event) => setRank(event.target.value)}>{data.ranks.map((item) => <option key={item}>{item}</option>)}</select></header>
        <div className="permission-groups">{groups.map((group) => <fieldset key={group}><legend>{group}</legend>{data.catalog.filter((permission) => permission.group === group).map((permission) => <label key={permission.key}><input type="checkbox" checked={permission.required || rankPermissions.has(permission.key)} disabled={permission.required} onChange={(event) => { const next = new Set(rankPermissions); if (event.target.checked) next.add(permission.key); else next.delete(permission.key); setData({ ...data, rankSettings: { ...data.rankSettings, [rank]: [...next] } }); }}/><span>{permission.label}{permission.required && <small>Required for every employee</small>}</span></label>)}</fieldset>)}</div>
      ...
      <article className="content-card permission-section"><header><div><h2>Employee exceptions</h2><p>Add or subtract access for one employee without changing their rank.</p></div><select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)}>{data.employees.map((employee) => <option value={employee.id} key={employee.id}>{employee.name} · {employee.rank}</option>)}</select></header>
        ...
        <div className="permission-override-list">{data.catalog.map((permission) => { const effect = permission.required ? "allow" : data.overrides[employeeId]?.[permission.key] ?? "rank"; const baseAllowed = Boolean(selectedEmployee?.isAdmin) || employeeRankPermissions.has(permission.key); return <label key={permission.key}><span><b>{permission.label}</b><small>{permission.required ? "Required for every employee" : `${selectedEmployee?.isAdmin ? "Administrator default" : "Rank"}: ${baseAllowed ? "Allowed" : "Not allowed"}`}</small></span><select value={effect} disabled={permission.required} onChange={(event) => { const next = { ...(data.overrides[employeeId] ?? {}) }; if (event.target.value === "rank") delete next[permission.key]; else next[permission.key] = event.target.value as "allow" | "deny"; setData({ ...data, overrides: { ...data.overrides, [employeeId]: next } }); }}><option value="rank">Use {selectedEmployee?.isAdmin ? "administrator default" : "rank setting"}</option><option value="allow">Add permission</option><option value="deny">Remove permission</option></select></label>; })}</div>
        <footer><button className="primary-action" disabled={saving} onClick={() => void save({ scope: "employee", employeeId, overrides: data.overrides[employeeId] ?? {} })}>Save employee exceptions</button></footer>
      </article>
    </> : <article className="content-card test-member-card"><span className="test-badge">ADMINISTRATOR TEST TOOL</span><h2>Preview the portal as a member</h2><p>This changes what you see and does not sign you in as the employee. Most areas remain read only so administrators can verify member access safely.</p><select value={testEmployeeId} onChange={(event) => onTestEmployee(data.employees.find((employee) => employee.id === event.target.value) ?? null)}><option value="">Choose a member…</option>{data.employees.map((employee) => <option value={employee.id} key={employee.id}>{employee.name} · {employee.rank}</option>)}</select>{testEmployeeId && <button onClick={() => onTestEmployee(null)}>Exit test view</button>}<small>This tab can be hidden later without deleting permission settings.</small></article>}
