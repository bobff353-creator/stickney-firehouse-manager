...
export default function StaffingRotation({
  ...
  const controls = <div className="staffing-rotation-controls" aria-label="Staffing panel pages">{views.map((view, index) => <button key={view.type === "new-member" ? view.member.id : view.type} className={index === activeIndex ? "active" : ""} aria-label={`Show ${view.type === "current" ? "current staffing" : view.type === "schedule" ? "department schedule for the next 24 hours" : `${formatEmployeeName(view.member.name)} new member spotlight`}`} onClick={() => setViewIndex(index)} />)}<span>Rotates every 10 seconds</span></div>;

  if (mode === "board") {
    return <section className={`board-panel staffing staffing-rotation-panel ${current.type}`} aria-live="polite"><header><h2>{title}</h2><span>{subtitle}</span></header><div className="staffing-rotation-body">{content}</div><footer>{controls}</footer></section>;
  }

  return <article className={`command-card on-duty staffing-rotation-card ${current.type}`} aria-live="polite"><header><span className="command-icon">{current.type === "current" ? "●" : current.type === "schedule" ? "◷" : "+"}</span><div><small>{title}</small><h2>{subtitle}</h2></div></header><div className="staffing-rotation-body">{content}</div><footer>{controls}{current.type === "current" && onOpenDailyLog ? <button onClick={onOpenDailyLog}>Open Daily Log →</button> : null}</footer></article>;
}
