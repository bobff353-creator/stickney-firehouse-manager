...
export default function OsfmTaskBrowser({selected=[],onChange,onUseBook,onTraining}:Props) {
  const [query,setQuery]=useState(''),[certId,setCertId]=useState(''),[chosenBook,setChosenBook]=useState(''),[taskQuery,setTaskQuery]=useState(''),[preview,setPreview]=useState(false);
  ...
  function pick(id:string){setCertId(id);setChosenBook('');setTaskQuery('');setPreview(false);}
  ...
  return <section className="tr-osfm-browser" aria-label="OSFM task-book picker">
    ...
    {cert&&<section ref={detailRef} className="tr-panel" aria-label="Selected official task book"><button type="button" className="tr-back" onClick={()=>catalogRef.current?.scrollIntoView({block:'center'})}>← Back to certifications</button><div className="tr-heading"><div><small>Illinois OSFM · Rule §{cert.section}</small><h3>{cert.title}</h3></div><a href={cert.ruleUrl} target="_blank" rel="noreferrer">Official requirements ↗</a></div>
      {available.length?<><div className="tr-inline-actions">{available.map(b=><button key={bookKey(b)} type="button" aria-pressed={book===b} onClick={()=>{setChosenBook(bookKey(b));setTaskQuery('');setPreview(false);}}>{bookKindLabel(b)}</button>)}</div>
        {book&&<><p><strong>{book.edition}</strong> · {book.bookDate||'Date shown in official book'} · {book.pageCount} pages</p>
          <div className="tr-inline-actions"><a href={book.url} target="_blank" rel="noreferrer">Open official PDF ↗</a><button type="button" onClick={()=>setPreview(!preview)}>{preview?'Close book preview':'Read / scroll task book'}</button>{onUseBook&&<button type="button" onClick={()=>onUseBook(book)}>Use this task book</button>}</div>
          {preview&&<><iframe className="tr-book-preview" title={`${book.title} ${bookKindLabel(book)} official PDF`} src={book.url} loading="lazy"/><p className="tr-muted">If your browser cannot display the PDF here, use Open official PDF above.</p></>}
