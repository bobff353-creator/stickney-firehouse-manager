        ...
        {loading && !data ? <PortalSkeleton page={activeNav} /> : data && <>
          ...
          {activeNav === "Scheduling" && <div className="schedule-workspace-shell">
            <nav className="schedule-page-tabs" aria-label="Scheduling workspace">
              {(isAdminView && !scheduleEmployeePreview ? [["calendar","Schedule"],["requests","Requests & trades"],["people","Personnel"],["patterns","Rules & reminders"]] : [["calendar","My schedule"],["open","Open shifts & trades"],["alerts","Reminders"]]).map(([key,label], index) => <button key={`${key}-${label}-${index}`} className={schedulingSection === key ? "active" : ""} aria-pressed={schedulingSection === key} onClick={() => openSchedulingSection(key)}>{label}</button>)}
