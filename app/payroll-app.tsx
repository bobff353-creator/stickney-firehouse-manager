...
import { matchesWorkspace } from "./workspace-help";
...
export default function PayrollApp({
  ...
  const [employeeRateContext, setEmployeeRateContext] = useState<{ name: string; scaleId: string } | null>(null);
  ...
  const globalSearchResults = useMemo(() => {
    ...
    return [...screens, ...employeeItems, ...sharedSearchItems].filter((item) => permittedPages.includes(item.page) && (item.type === "Screen" ? matchesWorkspace(item.page, term) : `${item.type} ${item.title} ${item.detail}`.toLowerCase().includes(term))).slice(0, 30);
  ...
  const visibleMoreNavGroups = useMemo(() => {
    ...
    const groups = adminNavGroups.map((group) => {
      const items = group.items.filter((item) => visibleNav.includes(item.page) && !featuredNavPages.has(item.page));
      items.forEach((item) => groupedPages.add(item.page));
      return { ...group, items };
    }).filter((group) => group.items.length);
    const ungrouped = visibleNav.filter((page) => !featuredNavPages.has(page) && !groupedPages.has(page));
    if (ungrouped.length) groups.unshift({ label: "Personal", icon: "users", items: ungrouped.map((page) => ({ label: page, page })) });
    return groups;
  }, [visibleNav]);
  ...
  function navigate(page: NavItem, record?: PortalRecord, fromBack = false) {
    ...
    setProfileOpen(false);
    setInventoryError("");
    if (page !== "Rates & Rules") setEmployeeRateContext(null);
    if (page === "Inventory") {
    ...
    <WorkspaceViewMemory key={`${access.identity}:${testMember?.id ?? "self"}`}><main className={`app-shell${tvMode ? " tv-shell" : ""}${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
      ...
      <aside id="desktop-navigation" className="desktop-sidebar" aria-hidden={sidebarCollapsed} inert={sidebarCollapsed}>
        ...
        <nav className="sidebar-nav" aria-label="Primary navigation">
          ...
          {visibleMoreNavGroups.length ? <details className="desktop-more-nav" open={moreToolsOpen} onToggle={(event) => setMoreToolsOpen(event.currentTarget.open)}>
            <summary><Icon name="menu" size={17}/><span>More tools</span><Icon name="chevron" size={14}/></summary>
            <div className="desktop-more-nav-groups">{visibleMoreNavGroups.map((group) => <section key={group.label}><h2>{group.label}</h2>{group.items.map((item) => <button key={item.page} aria-current={activeNav === item.page ? "page" : undefined} className={activeNav === item.page ? "current" : ""} onClick={() => navigate(item.page)}><Icon name={navIcons[item.page]}/><span>{item.label}</span></button>)}</section>)}</div>
          </details> : null}
      ...
      {mobileMenuOpen && <nav id="mobile-navigation" className="mobile-nav-panel" aria-label="Mobile navigation">
          ...
          {visibleMoreNavGroups.length ? <details className="mobile-more-nav" open={moreToolsOpen} onToggle={(event) => setMoreToolsOpen(event.currentTarget.open)}><summary><Icon name="menu" size={18}/><span>More tools</span><Icon name="chevron" size={14}/></summary><div>{visibleMoreNavGroups.map((group) => <section key={group.label}><h2>{group.label}</h2>{group.items.map((item) => <button key={item.page} aria-current={activeNav === item.page ? "page" : undefined} className={activeNav === item.page ? "current" : ""} onClick={() => navigate(item.page)}><Icon name={navIcons[item.page]}/>{item.label}</button>)}</section>)}</div></details> : null}
          ...
          {activeNav === "Employees" && data.viewer.canManageEmployees && <section className="employee-page">
            ...
            {profileOpen && <form className="content-card employee-profile-form" onSubmit={(event) => void saveEmployeeProfile(event)}>
              ...
              <fieldset id="employee-employment" tabIndex={-1}><legend>Employment</legend><div className="employee-photo-editor"><div className="employee-photo-preview">{employeePhotoPreview ? <img src={employeePhotoPreview} alt="Employee photo preview" /> : <span>{employeeNameFromParts(employeeDraft.lastName, employeeDraft.firstName).split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "FD"}</span>}</div><div><strong>Employee photo</strong><p>Used for new-member announcements and personnel displays.</p><div><label className="employee-photo-upload"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => chooseEmployeePhoto(event.target.files?.[0] ?? null)} /><span>{employeePhotoPreview ? "Choose a different photo" : "Choose photo"}</span></label>{employeePhotoPreview && <button type="button" className="quiet-button" onClick={() => { setEmployeePhotoFile(null); setEmployeePhotoPreview(""); setRemoveEmployeePhoto(Boolean(employeeDraft.id)); }}>Remove photo</button>}</div><small>JPG, PNG, or WebP · maximum 3 MB</small></div></div><div className="employee-fields three-col">
                ...
                <label className="admin-employee-check"><input disabled={!access.permissions.includes("permissions.manage")} type="checkbox" checked={employeeDraft.isAdmin} onChange={(event) => setEmployeeDraft((current) => ({ ...current, isAdmin: event.target.checked }))} /><span><strong>Portal administrator</strong><small>Controls app access, separately from rank and pay. Starts with access to every timesheet and payroll editing. Use Permissions → Employee exceptions to remove payroll access from a specific administrator.</small></span></label>
                <EmployeePayScale scales={data.payScales} value={employeeDraft.payScaleId} period={periodLabel(data.period.startDate, data.period.endDate)} canChangeScale={access.permissions.includes("permissions.manage")} canManageRates={isPayrollManagerView} employeeDirty={employeeDirty} onChange={payScaleId => setEmployeeDraft(current => ({ ...current, payScaleId }))} onEditRates={() => { if (employeeDirty || !isPayrollManagerView) return; navigate("Rates & Rules"); setEmployeeRateContext({ name: employeeNameFromParts(employeeDraft.lastName, employeeDraft.firstName), scaleId: employeeDraft.payScaleId }); }} />
              ...
              <fieldset id="employee-emergency" tabIndex={-1}><legend>Emergency contact</legend><div className="employee-fields three-col">
                ...
                <label><span>Emergency phone</span><input type="tel" inputMode="tel" placeholder="(708) 555-0123" value={employeeDraft.emergencyPhone} onChange={(event) => setEmployeeDraft((current) => ({ ...current, emergencyPhone: event.target.value }))} onBlur={() => setEmployeeDraft((current) => ({ ...current, emergencyPhone: formatPhoneNumber(current.emergencyPhone) }))} /></label>
              </div></fieldset>
