...
type Policy = { id: string; title: string; policyNumber: string; category: string; effectiveDate: string; body: string; updatedAt?: string; updatedBy?: string };
type BoxCard = { id: string; title: string; address: string; boxNumber: string; accessNotes: string; details: string; updatedAt?: string; updatedBy?: string };
...
function SharedPage({ type }: { type: "policy" | "boxCard" }) {
  ...
  return <section className="resource-page">
    ...
    <div className="resource-list">{filtered.length ? filtered.map((item) => isPolicy ? <article className="content-card resource-record" key={item.id}><div className="resource-record-head"><div><span>{(item as Policy).category || "General"}{(item as Policy).policyNumber ? ` · ${(item as Policy).policyNumber}` : ""}</span><h2>{item.title}</h2>{(item as Policy).effectiveDate && <time>Effective {(item as Policy).effectiveDate}</time>}</div>{canEdit && <button className="edit-employee" onClick={() => setDraft({ ...(item as Policy) })}>Edit</button>}</div><div className="resource-copy">{(item as Policy).body || "No policy text has been entered."}</div><p className="record-audit">Last updated {item.updatedAt ? new Date(item.updatedAt).toLocaleString() : "—"} · Updated by {item.updatedBy || "—"}</p></article> : <article className="content-card resource-record" key={item.id}><div className="resource-record-head"><div><span>Box Card{(item as BoxCard).boxNumber ? ` · ${(item as BoxCard).boxNumber}` : ""}</span><h2>{item.title}</h2>{(item as BoxCard).address && <p>{(item as BoxCard).address}</p>}</div>{canEdit && <button className="edit-employee" onClick={() => setDraft({ ...(item as BoxCard) })}>Edit</button>}</div>{(item as BoxCard).accessNotes && <div className="access-note"><strong>Access</strong><p>{(item as BoxCard).accessNotes}</p></div>}<div className="resource-copy">{(item as BoxCard).details || "No card details have been entered."}</div><p className="record-audit">Last updated {item.updatedAt ? new Date(item.updatedAt).toLocaleString() : "—"} · Updated by {item.updatedBy || "—"}</p></article>) : <div className="content-card resource-empty">No matching {isPolicy ? "policies" : "Box Cards"}.</div>}</div>
