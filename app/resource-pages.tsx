...
function BoxCardSheet({ card }: { card: BoxCard }) {
  ...
  if (!hasGrid && card.documentUrl) return <div className="box-card-image-wrap official-card-image"><Image unoptimized width={1980} height={1530} className="box-card-image" src={`/box-cards/stickney-${card.documentPage || 1}.jpg`} alt={`${card.department} Box Card ${card.boxNumber}: ${card.title}`} /></div>;
