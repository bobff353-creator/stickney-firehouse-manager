...
type BoxCard = AuditFields & { id: string; title: string; address: string; boxNumber: string; accessNotes: string; details: string; department: string; documentUrl?: string; documentPage?: number; effectiveDate?: string; reviewDate?: string };

const emptyPolicy: Policy = { id: "", title: "", policyNumber: "", category: "General", effectiveDate: "", body: "" };
const emptyBoxCard: BoxCard = { id: "", title: "", address: "", boxNumber: "", accessNotes: "", details: "", department: "Stickney", effectiveDate: "", reviewDate: "" };
  ...
  return <section className="resource-page">
    ...
    {draft && <form className="content-card resource-form" onSubmit={(event) => void save(event)}>
      ...
      {isPolicy ? (() => { const value = draft as Policy; return <div className="resource-form-grid">
        ...
        <label className="resource-body"><span>Special instructions</span><textarea rows={5} value={value.accessNotes} onChange={(event) => setDraft({ ...value, accessNotes: event.target.value })} placeholder="Dispatch, staging, callback, coverage, or change-of-quarters instructions…" /></label>
        <label className="resource-body"><span>Response plan</span><textarea rows={16} value={value.details} onChange={(event) => setDraft({ ...value, details: event.target.value })} placeholder="Enter Still, Full Still, Box, 2nd, 3rd, 4th, and interdivisional assignments…" /></label>
      </div>; })()}
    ...
    {!isPolicy && selectedDepartment && <div className="box-card-browser">
      ...
      <div className="policy-library box-card-library">
        ...
        {selectedBoxCard ? <article className="content-card resource-record official-record box-card-viewer"><div className="resource-record-head"><div><span>{selectedDepartment} · Box Card {selectedBoxCard.boxNumber}</span><h2>{selectedBoxCard.title}</h2>{selectedBoxCard.address && <p>{selectedBoxCard.address}</p>}</div><div className="box-card-actions">{canEdit && <button className="primary-action compact" onClick={() => setDraft({ ...selectedBoxCard })}>Edit Box Card</button>}</div></div><div className="box-card-meta"><div><span>Location / area</span><strong>{selectedBoxCard.address || "Not entered"}</strong></div><div><span>Effective date</span><strong>{selectedBoxCard.effectiveDate || "Not entered"}</strong></div><div><span>Review date</span><strong>{selectedBoxCard.reviewDate || "Not entered"}</strong></div><div><span>Status</span><strong>{selectedBoxCard.status || "Active"}</strong></div></div>{selectedBoxCard.accessNotes && <section className="box-card-section box-card-instructions"><h3>Special instructions</h3><div className="resource-copy">{selectedBoxCard.accessNotes}</div></section>}<section className="box-card-section"><h3>Response plan</h3><div className="resource-copy">{selectedBoxCard.details || "Administration has not entered the formatted response plan yet. Use Edit Box Card to add it."}</div></section>{selectedBoxCard.documentUrl && <details className="box-card-source"><summary>View original approved box card</summary><div className="box-source-actions"><a className="primary-action compact" href={`/box-cards/stickney-${selectedBoxCard.documentPage || 1}.jpg`} target="_blank" rel="noreferrer">Open full size</a><a className="quiet-button" href={`${selectedBoxCard.documentUrl}#page=${selectedBoxCard.documentPage || 1}`} target="_blank" rel="noreferrer">Original PDF</a></div><div className="box-card-image-wrap"><Image unoptimized width={1980} height={1530} className="box-card-image" src={`/box-cards/stickney-${selectedBoxCard.documentPage || 1}.jpg`} alt={`${selectedDepartment} Box Card ${selectedBoxCard.boxNumber}: ${selectedBoxCard.title}`} /></div></details>}<RecordCredibility audit={{ recordNumber: `BOX-${selectedBoxCard.boxNumber || selectedBoxCard.id.slice(0, 8).toUpperCase()}`, status: selectedBoxCard.status || "Active", createdBy: selectedBoxCard.createdBy, createdAt: selectedBoxCard.createdAt, updatedBy: selectedBoxCard.updatedBy, updatedAt: selectedBoxCard.updatedAt, revisions: selectedBoxCard.revisions }} /></article> : <div className="content-card box-select-prompt"><span aria-hidden="true">BOX</span><div><strong>Select a box card</strong><p>Choose a card from the list to open the formatted, editable response record.</p></div></div>}
      </div>
    </div>}
