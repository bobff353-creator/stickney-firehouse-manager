    ...
    <nav className="content-card policy-toc" aria-label="Policy table of contents">
      ...
      <div className="policy-toc-list">
        {policies.map((policy) => {
          ...
          return <button data-test-safe className={isCurrent ? "current" : ""} key={policy.id} type="button" aria-current={isCurrent ? "page" : undefined} onClick={() => onSelect(policy.id)}>
...
function SharedPage({ type }: { type: "policy" | "boxCard" }) {
  ...
  return <section className="resource-page">
    ...
    {!isPolicy && !selectedDepartment && <div className="box-department-grid">{departments.map((department) => {
      ...
      return <button data-test-safe className="content-card box-department-card" type="button" key={department} onClick={() => { setSelectedDepartment(department); setSelectedBoxCardId(""); setSearch(""); }}><span aria-hidden="true">BOX</span><div><strong>{department}</strong><small>{count} box {count === 1 ? "card" : "cards"}</small></div><b aria-hidden="true">›</b></button>;
    })}</div>}
