...
export default function OperationalPreplanPanel({preplanId,canEdit=false}:{preplanId:string;canEdit?:boolean}){
  ...
  const [reloadKey,setReloadKey]=useState(0),[editor,setEditor]=useState<""|"level"|"space"|"alert"|"hazmat"|"annotation"|"hoseLay"|"risk">(""),[saving,setSaving]=useState(false),[saveError,setSaveError]=useState("");
  const blankForm={name:"",shortLabel:"",aliases:"",severity:"warning",title:"",message:"",effectiveAt:"",expiresAt:"",materialName:"",unNumber:"",ergGuideNumber:"",quantity:"",quantityUnit:"",storageType:"",annotationType:"road_closure",label:"",totalDistanceFeet:"",hoseSizeInches:"4",sectionLengthFeet:"100",reserveFeet:"100",apparatusCapacityFeet:"",factor:"",score:"",explanation:"",source:"",targetHazardOverride:""};
  ...
  async function save(){
    ...
    try{
      const actions={level:"saveLevel",space:"saveSpace",alert:"saveAlert",hazmat:"saveHazmat",annotation:"saveAnnotation",hoseLay:"saveHoseLay",risk:"saveRiskFactor"} as const;
      if(!editor)return;
      ...
      if(editor==="hoseLay")Object.assign(payload,{totalDistanceFeet:Number(form.totalDistanceFeet),hoseSizeInches:Number(form.hoseSizeInches),sectionLengthFeet:Number(form.sectionLengthFeet),reserveFeet:Number(form.reserveFeet),apparatusCapacityFeet:form.apparatusCapacityFeet?Number(form.apparatusCapacityFeet):undefined,path:[],segmentDistances:[]});
  ...
  return <section className="operational-preplan-panel" aria-label="Operational preplan intelligence">
    ...
    {canEdit&&<div className="operational-editor-actions"><span>Add verified operational data</span>{([['level','Level'],['space','Room / area'],['alert','Response alert'],['hazmat','HazMat'],['annotation','Tactical annotation'],['hoseLay','Hose lay'],['risk','Risk factor']] as const).map(([key,name])=><button type="button" className={editor===key?"active":""} key={key} onClick={()=>{setEditor(editor===key?"":key);setSaveError("");}}>+ {name}</button>)}</div>}
    {editor&&<form className="operational-inline-editor" onSubmit={(event)=>{event.preventDefault();void save();}}><header><strong>{editor==="level"?"Add building level":editor==="space"?"Add room or operational area":editor==="alert"?"Add time-aware response alert":editor==="hazmat"?"Add HazMat inventory":editor==="annotation"?"Add named tactical annotation":editor==="hoseLay"?"Calculate and save hose lay":"Add transparent target-hazard factor"}</strong><button type="button" onClick={()=>setEditor("")} aria-label="Close editor">×</button></header>
      ...
      {editor==="hoseLay"&&<><label>Hose-lay name<input required value={form.name} onChange={(event)=>setForm({...form,name:event.target.value})} placeholder="Hydrant to Alpha side"/></label><label>Measured route<input required type="number" min="0" step="1" value={form.totalDistanceFeet} onChange={(event)=>setForm({...form,totalDistanceFeet:event.target.value})}/><small>feet</small></label><label>Hose size<input required type="number" min="0.5" step="0.5" value={form.hoseSizeInches} onChange={(event)=>setForm({...form,hoseSizeInches:event.target.value})}/><small>inches</small></label><label>Section length<input required type="number" min="1" step="1" value={form.sectionLengthFeet} onChange={(event)=>setForm({...form,sectionLengthFeet:event.target.value})}/><small>feet</small></label><label>Reserve<input required type="number" min="0" step="1" value={form.reserveFeet} onChange={(event)=>setForm({...form,reserveFeet:event.target.value})}/><small>feet</small></label><label>Verified apparatus capacity<input type="number" min="0" step="1" value={form.apparatusCapacityFeet} onChange={(event)=>setForm({...form,apparatusCapacityFeet:event.target.value})} placeholder="Leave blank if unverified"/><small>feet</small></label></>}
    ...
    <div className="operational-response-grid">
      <article><h4>Tactical annotations</h4>{data.annotations.filter((item)=>!item.archived&&(!item.levelId||!levelId||item.levelId===levelId)).length?data.annotations.filter((item)=>!item.archived&&(!item.levelId||!levelId||item.levelId===levelId)).map((item)=><section key={item.id}><div><strong>{label(item.name)}</strong><b>{label(item.annotationType).replaceAll("_"," ").toUpperCase()}</b></div><p>{label(item.label,"Named annotation awaiting mapped geometry")}</p></section>):<p className="operational-empty">No tactical annotations are recorded for this level.</p>}</article>
    ...
    {hoseLays.length>0&&<article className="operational-hose-lays"><h4>Measured hose lays</h4>{hoseLays.map((item)=>{const recommended=Number(item.recommendedHoseFeet??0),capacity=item.apparatusCapacityFeet==null?null:Number(item.apparatusCapacityFeet);return <section key={item.id}><div><strong>{label(item.name)}</strong><b>{recommended.toLocaleString()} FT RECOMMENDED</b></div><p>{Number(item.totalDistanceFeet??0).toLocaleString()} ft route + {Number(item.reserveFeet??0).toLocaleString()} ft reserve · {label(item.hoseSizeInches)}-inch hose</p><small>{capacity==null?"Apparatus inventory unverified":recommended<=capacity?`Verified capacity ${capacity.toLocaleString()} ft · sufficient`:`DEFICIT: ${(recommended-capacity).toLocaleString()} ft beyond verified capacity`}</small></section>})}</article>}
  </section>;
}
