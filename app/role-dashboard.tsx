...
export default function RoleDashboard({ data, onNavigate }: { data: DashboardData; onNavigate: (page: "Payroll" | "Daily Log" | "My Timesheet" | "Employees" | "Policies" | "Box Cards") => void }) {
  ...
  return <section className="role-dashboard">
    ...
    <section className="dashboard-quick"><h2>Quick access</h2><div><button onClick={() => onNavigate(data.viewer.isAdmin ? "Payroll" : "My Timesheet")}><span>◷</span><strong>{data.viewer.isAdmin ? "Payroll" : "My Timesheet"}</strong><small>Review current hours and pay</small></button><button onClick={() => onNavigate("Policies")}><span>▤</span><strong>Policies</strong><small>Search department policies</small></button><button onClick={() => onNavigate("Box Cards")}><span>⌂</span><strong>Box Cards</strong><small>Find building response information</small></button><a href="/draft"><span>🏈</span><strong>Draft Advisor</strong><small>Fantasy football draft board</small></a></div></section>
  </section>;
}
