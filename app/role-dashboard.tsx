...
export default function RoleDashboard({ data, onNavigate, allowedPages }: { data: DashboardData; onNavigate: (page: PortalPage, record?: PortalRecord) => void; allowedPages: readonly PortalPage[] }) {
  ...
  const nextShift = briefing?.nextShift?.employeeId === data.viewer.employeeId ? briefing.nextShift : null;
  const currentWork = [
  ...
  return <section className="role-dashboard">
    ...
    {briefing && <section className="command-status-grid" aria-label={briefingError ? "Last received department status — not current" : "Current department status"}>
      ...
      <article className={`command-card approvals ${pending ? "needs-attention" : "is-clear"}`}><header><span className="command-icon">{pending || "✓"}</span><div><small>Awaiting approval</small><h2>{pending ? `${pending} open item${pending === 1 ? "" : "s"}` : "All caught up"}</h2></div></header><div className="approval-lines">{allowedPages.includes("Daily Log") ? <button onClick={() => onNavigate("Daily Log")}><span>Log handoffs</span><strong>{briefing?.approvals.logs ?? 0}</strong></button> : <p>Log handoffs: {briefing?.approvals.logs ?? 0} · Review requires Daily Log access.</p>}{allowedPages.includes("Payroll") && <button onClick={() => onNavigate("Payroll")}><span>Payroll periods</span><strong>{briefing?.approvals.payroll ?? 0}</strong></button>}</div></article>
      <article className="command-card previous-shift"><header><span className="command-icon">↶</span><div><small>Previous shift</small><h2>{briefing ? shiftLabel(briefing.priorShift ?? "") : "Handoff summary"}</h2></div></header><blockquote>{briefing?.previousShift.note || "No previous-shift handoff note recorded."}</blockquote><div><span>{briefing?.previousShift.calls.length ?? 0} calls recorded</span>{briefing?.previousShift.officer && <span>OIC: {displayName(briefing.previousShift.officer)}</span>}</div>{briefing?.previousShift.calls.slice(0, 2).map((call, index) => <p className="previous-call" key={`${call.reportNumber}-${index}`}><strong>{call.callType}</strong> {call.timeOut ? formatMilitaryTime(call.timeOut) : "Time not entered"} · {call.respondingUnits || "Units not entered"}{call.address ? ` · ${call.address}` : ""}</p>)}</article>
    </section>}
