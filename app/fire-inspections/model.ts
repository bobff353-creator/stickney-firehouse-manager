...
export type InspectionCheck = { id: string; section: string; label: string; source: string; result: CheckResult; location: string; observation: string; correction: string; code: string; dueDate: string; correctedDate: string; priority: string };
...
export type InspectionData = {
  ...
  representative: Signature; inspectorAttested: boolean; changeReason: string; test: boolean;
};
...
export type InspectionFile = { id: string; recordId: string; filename: string; size: number; contentType: string; createdAt: string };
export type Property = { id: string; name: string; address: string };
export type InspectionSnapshot = { records: InspectionRecord[]; properties: Property[]; attachments: InspectionFile[] };
...
export function normalizeInspection(value: unknown): InspectionData {
  ...
  d.checks=v.checks.map(item=>{if(!item||typeof item!=='object')throw Error('Invalid checkpoint.');const c=item as Record<string,unknown>;const next={} as InspectionCheck;for(const k of Object.keys(freshCheck({id:'',label:'',section:'',source:''})) as (keyof InspectionCheck)[])(next as unknown as Record<string,string>)[k]=text(c[k],k==='id'?100:4000);if(!next.id||!next.label||!resultOptions.includes(next.result))throw Error('Each checkpoint needs a label and a valid result.');if(!['Routine','Urgent','Immediate action'].includes(next.priority))throw Error('Choose a valid finding priority.');return next;});
  if(new Set(d.checks.map(c=>c.id)).size!==d.checks.length)throw Error('Duplicate checkpoints are not allowed.');
  const s=(v.representative??{}) as Partial<Signature>;
  ...
  d.representative={name:text(s.name,200),role:text(s.role,160),state:s.state??'Not requested',strokes,signedAt:text(s.signedAt,50)};
  if(d.representative.state!=='Signed'){d.representative.strokes=[];d.representative.signedAt='';}
  return d;
}
export function updateInspectionDetail<K extends keyof InspectionData>(d:InspectionData,key:K,value:InspectionData[K]):InspectionData {
 return {...d,[key]:value,representative:key==='representative'?value as Signature:key==='inspectorAttested'?d.representative:blankSignature(),inspectorAttested:key==='inspectorAttested'?value as boolean:false};
}
...
export function validateInspection(kind:InspectionRecord['kind'],d:InspectionData,today=todayChicago()) {
  ...
  for(const t of [d.scheduledTime,d.startTime,d.endTime])if(t&&!/^([01]\d|2[0-3]):[0-5]\d$/.test(t))throw Error('Use 24-hour time, such as 09:00 or 14:30.');
  ...
  if(d.representative.state==='Signed'&&(!d.representative.name||!d.representative.strokes.some(stroke=>stroke.length>1)))throw Error('Enter the signer’s name and have the representative draw their signature.');
