...
export default function AlternativeReviewEditor({value:r,onChange,files,onUpload}:{value:AlternativeReview;onChange:(v:AlternativeReview)=>void;files:InspectionFile[];onUpload:(file:File,caption:string)=>Promise<void>}){
 ...
 const field=(key:keyof AlternativeReview,label:string,multiline=false,type='text',hint='')=><label className="fi-field">{label}{multiline?<textarea value={String(r[key])} onChange={e=>set(key,e.target.value as never)}/>:<input type={type} value={String(r[key])} onChange={e=>set(key,e.target.value as never)} onInput={type==='date'?e=>set(key,e.currentTarget.value as never):undefined}/>} {hint&&<small>{hint}</small>}</label>;
