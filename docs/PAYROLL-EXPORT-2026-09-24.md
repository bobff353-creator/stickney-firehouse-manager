...
- Source `a860f59dd5b6a20da84f687799b5e54816b36274` pushed on `codex/payroll-reference-export-20260924`.
- Deployment `dpl_5chxi5WaNjwx4Fvo7RzsiWn7XYPk` reached READY with no alias error on `https://stickney-firehouse-manager.vercel.app/`. CLI publishing required the explicit `fire-pre-plan-pro` scope.
- Authenticated production Payroll showed Export Excel and CSV. Both completed generation and displayed download-started notices for all 44 employees in September 11–25. The displayed gross remained $44,269.70. No browser warnings or errors were reported.
- The browser automation download-event listener timed out, so receipt of a saved live download file was not independently confirmed. XLSX contents, formulas, colors, totals, and reopen behavior were verified using a fictional file generated by the same source function.
- The updated Payroll page was left available in a separate tab; the user's employee screen and Chrome reference workbook were preserved.
