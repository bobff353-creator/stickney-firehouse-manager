...
| D — Hose lay | Segment totals, reserve and section rounding, independent supply options, verified capacity comparison, deficit, and unverified-not-zero behavior | Completed on the isolated preview with the user-confirmed Engine 1203 capacity, persisted route geometry, recommendation comparison, publication, Respond rendering, and cleanup | Complete on isolated preview |
...
- Cleanup: the dispatch was cleared, its projected Daily Log row was closed, and Respond returned to `No active call` with the controlled incident absent. The clearly labeled apparatus and hose-lay fixtures remain in the isolated preview as acceptance evidence.
...
- Branch and deployments: `codex/preplan-operational-v2-approved` started from `de5c4ee` on `dpl_DZkRAQD5ZifrJwvc1TpYK8ai2dLr`. SDS-linkage correction `913dc1a` deployed Ready as `dpl_8Dq638CdgfM7qq5TAC9qrjWHg5Ah` on the protected stable preview alias; the deployed editor exposed the required HazMat selector for SDS uploads.
...
- Checkpoint result: source selection, multi-segment distance persistence, total/recommendation calculation, reload, archive, and cleanup were verified; drawn route geometry and verified apparatus-capacity comparison were still pending at this checkpoint.

### 2026-08-21 — Scenario D drawn route geometry

- Implementation: commit `afb2849` added a normalized preplan drawing surface, required at least two bounded route points at the API boundary, persisted the path independently from measured distances, and displayed the saved geometry. Commit `37a232d` normalized database JSON text back into route points after the first live save exposed the mismatch.
- Deployment: `dpl_GBAz8Yoyc1toRYyR8C7uwebWcjVR` became Ready on the protected stable preview alias.
- Controlled record: `PREVIEW ONLY — Scenario D drawn route` saved three points plus independent segments of 120, 85, and 40 feet. The live editor calculated 245 feet total and 400 feet recommended with the configured reserve and section rounding.
- Persistence: after the normalization deployment, the authenticated preview API returned three path points, the three saved segments, 245 feet total, and 400 feet recommended. The Field panel rendered `Saved drawn route · 3 points` after reload.
- Apparatus boundary: the signed-in production inventory exposes nine apparatus profiles but no hose equipment record or verified hose-length capacity. Capacity therefore remains `Apparatus inventory unverified`; no value or apparatus record was fabricated or copied.
- Cleanup status: the controlled preview record remains active and clearly labeled pending acceptance of the browser's native archive confirmation. Production was not changed.
- Scenario D result: drawn route authoring and persistence are verified. Scenario D remains partial only because no authoritative apparatus hose-capacity record exists; final temporary-record cleanup is still pending.

### 2026-08-21 — Scenario E target-hazard review and Respond presentation

- Tester and scope: signed-in `User Preview Verification` administrator on the protected stable preview alias and isolated Supabase project `pzgvlslcaoqtrnaqyjmd`; production was not changed.
- Approved controlled factors: `PREVIEW ONLY — Bowstring truss` scored 20, `PREVIEW ONLY — Propane storage` scored 20, and `PREVIEW ONLY — Limited rear access` scored 15. Every factor included a firefighter-facing explanation and the clearly labeled Scenario E preview acceptance source.
- Authorized review and publication: an authorized reviewer applied a +5 manual classification override. The aggregate became 60 from factors totaling 55 plus the override, remained High, and was published as revision 4 with revision history increasing to three entries.
- Preview schema repair: Daily Log initially exposed the already-known additive migration gap for `daily_log_approvals.fleet_duties_acknowledged`. The existing idempotent repository migration's single column definition was applied only to the isolated preview and verified as integer, non-null, default 0. No production or unrelated migration was changed.
- Controlled Respond call: Daily Log call `PREVIEW-SCENARIO-E`, address `No production address`, unit `PREVIEW`, and type Fire matched the preview-only preplan by address.
- Respond evidence: published revision 4 displayed the prominent `TARGET HAZARD · HIGH` banner, all three readable reasons, and the transparent line `Published score 60 · Factors 55 · Authorized override +5` after commit `b7abbd4` deployed Ready as `dpl_GMQrC2d4yS1RwP5U4VrqVthX78h9`.
...
- Production inventory authorization: after the user authorized a source-preserving copy, the signed-in production portal's existing read-only preplan API was inspected. No database credential, contact value, photo content, or storage object was exposed, and no production mutation was attempted.
- Production inventory result: two legacy preplans exist. `Jewel Food Store` has a four-point footprint, three features, and calculated fire flow, but no contact information or photos. `Shell` has a six-point footprint and calculated fire flow, but no contact information, mapped features, or photos. Neither record has the required A–D photo set, so neither qualifies for the authorized copy.
- Scenario G result: automated migration contracts remain passing, but manual acceptance is blocked because production contains no single legacy record with contacts, footprint, features, A–D photos, and fire-flow evidence. No record was copied. Invented or combined data is not an acceptable substitute.
