...
| E — Target hazard | Explainable factor scoring, override validation, published Respond banner, and readable reasons | Remove the controlled Daily Log call after acceptance evidence is recorded | Manual verification complete on isolated preview; controlled call cleanup pending |
...
- Cleanup state: the controlled call and one blank call row created during the blocked first attempt remain pending explicit destructive-action confirmation. Scenario E manual behavior is verified but is not marked complete until those two preview-only call rows are removed or closed as approved.
