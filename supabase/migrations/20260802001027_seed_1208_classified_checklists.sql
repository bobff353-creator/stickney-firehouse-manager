...
begin
  ...
  if stickney_id is null or utility_1208_id is null then
    raise exception 'Stickney Fire Department apparatus 1208 must exist before loading form 267';
  end if;

  insert into public.inventory_compartments
    (department_id, apparatus_id, label, side, sort_order)
  values
    (stickney_id, utility_1208_id, 'Vehicle', 'cab', 100),
    (stickney_id, utility_1208_id, 'Disinfection / Cleaning', 'interior', 110),
    (stickney_id, utility_1208_id, 'Front Tires', 'front', 120),
    (stickney_id, utility_1208_id, 'Rear Tires', 'rear', 130),
    (stickney_id, utility_1208_id, 'Cab', 'cab', 200)
  ...
  with source_items(section_label, item_name, quantity_required, equipment_category, check_types, item_order) as (
    values
      ...
      ('Disinfection / Cleaning','Inspect all hand tools; clean and oil as needed',1,'vehicle',array['weekly'],2),
      ...
      ('Cab','Bell Fuels gas card',1,'equipment',array['inventory'],1),
      ('Cab','Insurance card',1,'equipment',array['inventory'],2),
      ('Cab','Box light',1,'equipment',array['inventory'],3),
      ('Cab','Registration',1,'equipment',array['inventory'],4),
      ('Cab','Fire extinguisher',1,'equipment',array['inventory'],5),
      ('Cab','Safety vests',2,'equipment',array['inventory'],6),
      ('Cab','Mobile radio',1,'equipment',array['inventory'],7),
      ('Cab','Knox Box keys',1,'equipment',array['inventory'],8),
      ('Cab','Door opener',1,'equipment',array['inventory'],9),
      ('Cab','I-PASS',1,'equipment',array['inventory'],10),
      ('Cab','Emergency Response Guidebook',1,'equipment',array['inventory'],11),
      ('Cab','Driver SCBA',1,'air_pack',array['air_pack'],12)
  ), ranked_items as (
    select
      source_items.*,
      c.id as compartment_id,
      'form-267-' || lpad(row_number() over (
