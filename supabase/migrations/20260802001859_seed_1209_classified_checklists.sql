...
begin
  ...
  if stickney_id is null or utility_1209_id is null then
    raise exception 'Stickney Fire Department apparatus 1209 must exist before loading form 269';
  end if;

  insert into public.inventory_compartments
    (department_id, apparatus_id, label, side, sort_order)
  values
    (stickney_id, utility_1209_id, 'Vehicle', 'cab', 100),
    (stickney_id, utility_1209_id, 'Disinfection / Cleaning', 'interior', 110),
    (stickney_id, utility_1209_id, 'Front Tires', 'front', 120),
    (stickney_id, utility_1209_id, 'Rear Tires', 'rear', 130),
    (stickney_id, utility_1209_id, 'Cab', 'cab', 200)
  ...
  with source_items(section_label, item_name, quantity_required, equipment_category, check_types, item_order) as (
    values
      ...
      ('Vehicle','Transmission fluid (vehicle must be on and in park)',1,'vehicle',array['weekly'],4),
      ...
      ('Disinfection / Cleaning','Sweep and mop rear bed of truck',1,'vehicle',array['weekly'],2),
      ...
      ('Cab','Mobile radio',1,'equipment',array['inventory'],1),
      ('Cab','Knox Box keys',1,'equipment',array['inventory'],2),
      ('Cab','Bell Fuels gas card',1,'equipment',array['inventory'],3),
      ('Cab','Insurance card',1,'equipment',array['inventory'],4),
      ('Cab','Registration',1,'equipment',array['inventory'],5),
      ('Cab','Safety vests',2,'equipment',array['inventory'],6),
