...
begin
  ...
  if stickney_id is null then
    raise exception 'Stickney Fire Department must exist before adding Utility 1209';
  end if;
  ...
  if utility_1209_id is null then
    insert into public.department_apparatus
      (department_id, unit_name, unit_type, call_sign, station, status, notes)
    values
      (stickney_id, '1209', 'Utility', '1209', 'Stickney Fire Department', 'in_service', '')
    returning id into utility_1209_id;
  else
