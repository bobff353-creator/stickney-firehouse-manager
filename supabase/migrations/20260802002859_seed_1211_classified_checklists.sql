...
begin
  ...
  where department_id = stickney_id and name = '1211'
  limit 1;

  if stickney_id is null or utility_1211_id is null then
    raise exception 'Stickney Fire Department apparatus 1211 must exist before loading form 466';
  end if;

  insert into public.inventory_compartments
    (department_id, apparatus_id, label, side, sort_order)
  values
    (stickney_id, utility_1211_id, 'Vehicle', 'cab', 100),
    (stickney_id, utility_1211_id, 'Disinfection / Cleaning', 'interior', 110),
    (stickney_id, utility_1211_id, 'Front Tires', 'front', 120),
    (stickney_id, utility_1211_id, 'Rear Tires', 'rear', 130),
    (stickney_id, utility_1211_id, 'Cab', 'cab', 200)
  ...
  with source_items(section_label, item_name, quantity_required, equipment_category, check_types, item_order) as (
    values
      ...
      ('Cab','Box light',1,'equipment',array['inventory'],2),
      ('Cab','Registration',1,'equipment',array['inventory'],3),
      ('Cab','Fire extinguisher',1,'equipment',array['inventory'],4),
      ('Cab','Safety vests',2,'equipment',array['inventory'],5),
      ('Cab','Mobile radio',1,'equipment',array['inventory'],6),
      ('Cab','Driver SCBA',1,'air_pack',array['air_pack'],7),
    ...
    join public.inventory_compartments c
     ...
     and c.apparatus_id = utility_1211_id
